#!/usr/bin/env python3
"""
Huurs Studio - Surah Al-Kahf Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/kahf_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/KAHF_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_KAHF_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH AL-KAHF", w/2 - 135, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE SANCTUARY OF TRUTH, THE FOUR TRIBULATIONS & THE SHIELD AGAINST DECEPTION", w/2 - 325, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of The Cave Companions, The Two Gardens, Musa & Khidr, and Dhul-Qarnayn Across 13 Foundation Lectures", w/2 - 382, 288, font="F1", size=8.5, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("FAITH & WEALTH TRIBULATIONS", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("ASHAB AL-KAHF & SAHIB AL-JANNATAYN", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("309-Year Solar Slumber, The Faithful Dog & The Shield Against Dajjal", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Hubris of Capital, Hand-Wringing Regret & Enduring Righteous Deeds", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("KNOWLEDGE & POWER TRIBULATIONS", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("MUSA & AL-KHIDR, DHUL-QARNAYN & FIRDAUS", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("The Three Epistemological Paradoxes: Ship, Slain Youth & Rebuilt Wall", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Advanced Metallurgy of Iron and Copper, Boundless Oceans of Divine Words", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AL-KAHF MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("TRIALS OF FAITH, CELESTIAL MECHANICS & WEALTH", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: Qayyiman, 309-Year Sleep, Two Gardens & The Unfolded Record", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Unwarped Scripture & The Refuge of Youth", "Plate 03", "Qayyiman, Dajjal Protection, Youth Standing & The Cave Refuge"),
    ("Plate 04 : Celestial Slumber & The Awakening in Time", "Plate 04", "309 Years, Solar Alignment, Somatic Turning & In Sha' Allah"),
    ("Plate 05 : The Parable of Two Gardens & Vanity of Wealth", "Plate 05", "Capitalistic Hubris, Faithful Admonition & Al-Baqiyat as-Salihat"),
    ("Plate 06 : Barren Earth, Unveiled Record & Rebellious Jinn", "Plate 06", "Cosmic Reckoning, Infallible Ledger, Iblis & Human Argument")
]
...
pdf.text("TRIALS OF KNOWLEDGE, POWER & INFINITE WORDS", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Musa & Khidr, Three Paradoxes, Dhul-Qarnayn & Oceans of Ink", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Quest for Knowledge & Prophetic Encounter", "Plate 07", "Musa's Resolve, Confluence of Seas, Escaping Fish & Ladunni Pact"),
    ("Plate 08 : Three Epistemological Paradoxes Unveiled", "Plate 08", "Damaged Ship, Slain Boy, Rebuilt Wall & Transcendent Wisdom"),
    ("Plate 09 : Sovereignty of Dhul-Qarnayn & The Iron Rampart", "Plate 09", "Expeditions, Refusing Bribes, Iron-Copper Wall & Sovereign Humility"),
    ("Plate 10 : The Greatest Losers, Ocean of Words & Tawhid", "Plate 10", "Delusion of Sincerity, Firdaus Lodging, Oceans of Ink & Verse 110"),
...
pdf.text("SURAH AL-KAHF FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/kahf_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/kahf_compendium_page-01.png", os.path.join(brain_dir, "kahf_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/kahf_compendium_page-02.png", os.path.join(brain_dir, "kahf_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/kahf_compendium_page-01.png", f"{PREVIEWS_DIR}/kahf_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/kahf_compendium_page-02.png", f"{PREVIEWS_DIR}/kahf_compendium_toc.png")
