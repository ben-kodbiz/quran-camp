#!/usr/bin/env python3
"""
Huurs Studio - Surah At-Tawbah Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/at_tawbah_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/AT_TAWBAH_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_AT_TAWBAH_MASTER_COMPENDIUM.pdf")
...
# Golden Title Header
pdf.text("SURAH AT-TAWBAH", w/2 - 165, 345, font="F2", size=32, rgb=GOLD)
...
# Subtitle
pdf.text("The Definitive Visual Knowledge Architecture of Moral Ultimatum, Hypocrisy Unmasked & Divine Repentance Across Classical Sunni Exegetical Foundations", w/2 - 325, 288, font="F1", size=9.5, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("MORAL ULTIMATUM & COVENANTS", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("BARA'AH & THE CAVE OF THAWR", w/2 - box_w - 5, 227, font="F2", size=11, rgb=WHITE)
pdf.text("The Omission of Basmalah & Four Months Respite (Faseehoo)", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("'Do Not Grieve, Allah is with Us' & The Illusion of Numbers at Hunayn", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=ROSE, line_width=1.0)
pdf.text("HYPOCRISY UNMASKED & EXPANSIVE TAWBAH", w/2 + 25, 243, font="F2", size=9.5, rgb=ROSE)
pdf.text("THE SACRED PURCHASE & THE THREE LEFT BEHIND", w/2 + 25, 227, font="F2", size=11, rgb=WHITE)
pdf.text("The Subversive Mosque (Dirar) vs Masjid Quba Founded on Taqwa", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Agony of Ka'b, Cosmic Constriction & Prophetic Mercy (Ra'oofun Raheem)", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AT-TAWBAH MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("ULTIMATUM, HUNAYN, TABUK & SOCIAL ARCHITECTURE", c1_x + 12, 365, font="F2", size=9, rgb=CYAN)
pdf.text("Plates 01 to 04: Bara'ah, The Cave of Thawr, Clinging to Earth & 8 Zakah Classes", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Divine Ultimatum & Sacred Sanctuary", "Plate 03", "Omission of Basmalah, Yawm al-Hajj al-Akbar, 4-month grace, Asylum rules"),
    ("Plate 04 : False Security, Hunayn & The Cave", "Plate 04", "Vanity of numbers at Hunayn, Descent of Sakinah, Thawr sanctuary, La Tahzan"),
    ("Plate 05 : The Crucible of Tabuk & Hypocrisy", "Plate 05", "Clinging to earth, March light/heavy, Mockery in jest, Rejected charity"),
    ("Plate 06 : Social Architecture of Nifaq & True Faith", "Plate 06", "Network of hypocrites, Forgetting God, Believers as Awliya, 8 Zakah recipients")
]
...
pdf.text("FALSE MOSQUES, DIVINE PURCHASE, TAWBAH & COMPASSION", c2_x + 12, 365, font="F2", size=9, rgb=GOLD)
pdf.text("Plates 05 to 08: Masjid ad-Dirar, Soul Purchase, The Three Left Behind & Hasbiyallah", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Sins of Miserliness & False Sanctuaries", "Plate 07", "Broken vows of wealth, Mocking charity, Masjid ad-Dirar vs Masjid Quba"),
    ("Plate 08 : The Sacred Transaction & Rectitude", "Plate 08", "Inna Allaha-shtara, 9 hallmarks of redeemed, Ibrahim's disassociation"),
    ("Plate 09 : Trial of Tabuk & The Three Left Behind", "Plate 09", "Ka'b/Hilal/Murarah, Boycott, Earth constricted, Repentance accepted"),
    ("Plate 10 : Scholarship, Purification & Prophetic Heart", "Plate 10", "Tafaqquh fid-Deen, Heart litmus test, Ra'oofun Raheem, Hasbiyallah"),
...
pdf.text("SURAH AT-TAWBAH FOUNDATION ARCHITECTURE", w - 250, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/at_tawbah_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/at_tawbah_compendium_page-01.png", os.path.join(brain_dir, "at_tawbah_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/at_tawbah_compendium_page-02.png", os.path.join(brain_dir, "at_tawbah_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/at_tawbah_compendium_page-01.png", f"{PREVIEWS_DIR}/at_tawbah_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/at_tawbah_compendium_page-02.png", f"{PREVIEWS_DIR}/at_tawbah_compendium_toc.png")
