#!/usr/bin/env python3
"""
Huurs Studio - Surah Al-Ankabut Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/al_ankabut_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/AL_ANKABUT_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_AL_ANKABUT_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH AL-ANKABUT", w/2 - 140, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE CRUCIBLE OF FAITH: THE SPIDER'S WEB & THE GUIDED PATHWAYS", w/2 - 290, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of The Furnace of Testing (A-Hasiba An-Nas), Parental Boundaries, Nuh's 950 Years, Ibrahim's Cooling Fire, The Master Parable of the Spider's House, The Restraining Prayer & The Striving Soul Across 4 Movements", w/2 - 380, 288, font="F1", size=7.2, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("THE CRUCIBLE & PROPHETIC PATIENCE", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("NUH, IBRAHIM & FILIAL BOUNDARIES", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Fitnah as Smelting Gold, Sa'd's Mother & No Shirk", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Nuh's 950-Year Steadfastness & Ibrahim's Fire Rescue", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("THE SPIDER'S WEB & DIVINE GUIDANCE", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("AWHANA AL-BUYOOT & JAHADOO FEENA", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("The Fragile Cobweb: Structural & Biological Toxicity", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Prayer Restraining Evil & We Will Surely Guide Them", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AL-ANKABUT MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("THE FURNACE OF TESTING & PROPHETS", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: A-Hasiba An-Nas, Sa'd's Trial, Nuh's 950 Years & Ibrahim's Fire", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Law of Tribulation & Parental Limits", "Plate 03", "A-Hasiba An-Nas, Fitnah as Smelting, Sa'd's Mother & Monotheism"),
    ("Plate 04 : Hypocrisy & The 950-Year Endurance of Nuh", "Plate 04", "Fair-Weather Soul, False Sin Pacts & Nuh's Ark Deliverance"),
    ("Plate 05 : Ibrahim's Furnace & The Sacred Hijrah", "Plate 05", "Denouncing Idols, Provision from Allah, Cooling Fire & Exile"),
    ("Plate 06 : Lut, Madyan & Retribution of Tyrants", "Plate 06", "Moral Courage, Angelic Rescue, Shu'ayb & The Four Retributions")
]
...
pdf.text("THE SPIDER'S WEB, SALAH & GUIDANCE", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Bayt al-Ankabut, The Living Prayer, Ummi Prophet & Striving", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : The Parable of the Spider's House", "Plate 07", "Bayt al-Ankabut, Structural Fragility, Biological Toxicity & Sages"),
    ("Plate 08 : The Spiritual Arsenal: Qur'an & Salah", "Plate 08", "Tilawah, Restraining Fahsha' and Munkar & Supreme Dhikr"),
    ("Plate 09 : Proofs of Prophethood & Scripture in Hearts", "Plate 09", "The Unlettered Prophet, Living Breasts & Scripture Sufficiency"),
    ("Plate 10 : Universal Mortality & The Guided Pathways", "Plate 10", "Dha'iqatu al-Mawt, Dunya as Play, Jahadoo Feena & Muhsineen"),
...
pdf.text("SURAH AL-ANKABUT FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/al_ankabut_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/al_ankabut_compendium_page-01.png", os.path.join(brain_dir, "al_ankabut_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/al_ankabut_compendium_page-02.png", os.path.join(brain_dir, "al_ankabut_compendium_toc.png"))
