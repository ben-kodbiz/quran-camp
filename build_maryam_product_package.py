#!/usr/bin/env python3
"""
Huurs Studio - Surah Maryam Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/maryam_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/MARYAM_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_MARYAM_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH MARYAM", w/2 - 120, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE SANCTUARY OF RAHMAH, MIRACULOUS PARENTAGE & THE REPUTATION OF TRUTH", w/2 - 320, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of Zakariyya, Yahya, Maryam, Isa, Ibrahim, and the Lineage of Guided Servants Across 3 Foundation Lectures", w/2 - 380, 288, font="F1", size=8.5, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("DIVINE MERCY & MIRACULOUS LINEAGE", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("ZAKARIYYA, YAHYA & MARYAM", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("The Secret Whisper, Barrenness Healed & Wisdom in Childhood", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Maternal Solitude Under the Date Palm & Infant Speech in the Cradle", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("FILIAL PIETY & CELESTIAL HORROR OF SHIRK", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("IBRAHIM, THE PROPHETS & COVENANT OF WUDD", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Four 'Ya Abati' Appeals, Parting in Peace & Truthful Posterity", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Cosmic Outrage at False Attribution & The Divine Endowment of Love", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH MARYAM MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("DIVINE MERCY, MIRACLES & PROPHETIC BIRTH", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: Zakariyya's Whisper, Yahya, Maryam's Solitude & Isa's Defense", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Supplication of Intimacy & Sign of Yahya", "Plate 03", "Zakariyya's Whisper, Old Age Vulnerability, Spiritual Heir & Silence"),
    ("Plate 04 : The Nativity of Isa & Solitude Under Palm", "Plate 04", "Sanctuary Retreat, Ruḥ Appearance, Agony Under Palm & Consolation"),
    ("Plate 05 : Infant Speech in Cradle & Truthful Word", "Plate 05", "Silent Fast, People's Accusation, Cradle Proclamation & Non-Divinity"),
    ("Plate 06 : Filial Compassion of Ibrahim & Parting Peace", "Plate 06", "Four 'Ya Abati' Calls, Azar's Stoning Threat & Truthful Posterity")
]
...
pdf.text("PROPHETIC INHERITANCE & DIVINE LOVE", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Prophetic Lineage, Later Generations, Cosmic Outrage & Wudd", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Prophetic Lineage & Tearful Prostration", "Plate 07", "Musa, Harun, Isma'il, Idris, Prostration in Tears & Neglectful Heirs"),
    ("Plate 08 : Resurrected Solitude & Intercession by Leave", "Plate 08", "Human Skepticism, Kneeling Before Hellfire & Saving Muttaqin"),
    ("Plate 09 : Cosmic Horror of False Attribution", "Plate 09", "Mountains Collapsing, Denial of Offspring & Solitary Return to Allah"),
    ("Plate 10 : The Divine Endowment of Love & Warning", "Plate 10", "Endowment of Wudd, Tongue of Ease, Joy to Righteous & Past Peoples"),
...
pdf.text("SURAH MARYAM FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/maryam_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/maryam_compendium_page-01.png", os.path.join(brain_dir, "maryam_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/maryam_compendium_page-02.png", os.path.join(brain_dir, "maryam_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/maryam_compendium_page-01.png", f"{PREVIEWS_DIR}/maryam_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/maryam_compendium_page-02.png", f"{PREVIEWS_DIR}/maryam_compendium_toc.png")
