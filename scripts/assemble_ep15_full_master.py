#!/usr/bin/env python3
"""
HUURS STUDIO - Episode 15 Full Master Assembly (Season 2)
=========================================================
Assembles all 5 living-creation scene clips for Episode 15 (Surah Al-Hijr 15:97–99)
...
- Section 1: Ocean Dawn with Rolling Sea Mist (The Wound of Sharp Words)
- Section 2: Full Moon & Stars over Desert Dunes (The Divine Validation & Cosmic Peace)
- Section 3: Taif Mountain Agricultural Terraces (Classical Tafsir & Deeq as-Sadr)
- Section 4: Magnificent Peacock in Courtyard Garden (The Tadabbur Shift & Healing in Sujood)
- Section 5: Sunrise over Ocean Coastline (The Sajdah of Relief & Boundless Peace)
...
AUDIO_IN = os.path.join(ROOT_DIR, "11_AUDIO/EP15_audio.mp3")
TIMINGS_JSON = os.path.join(ROOT_DIR, "11_AUDIO/EP15_audio_timings.json")
OUTPUT_MASTER = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP15-FULL-MASTER.mp4")
OUTPUT_SHORT = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP15-SHORT-001A.mp4")
TEMP_DIR = "/tmp/ep15_assembly"

os.makedirs(TEMP_DIR, exist_ok=True)

S1_FILE = os.path.join(RENDERS_DIR, "EP15_SCENE01.mp4")
S2_FILE = os.path.join(RENDERS_DIR, "EP15_SCENE02.mp4")
S3_FILE = os.path.join(RENDERS_DIR, "EP15_SCENE03.mp4")
S4_FILE = os.path.join(RENDERS_DIR, "EP15_SCENE04.mp4")
S5_FILE = os.path.join(RENDERS_DIR, "EP15_SCENE05.mp4")

DEFAULT_SECTIONS = [
    {"id": "sec1", "file": S1_FILE, "name": "Ocean Dawn with Sea Mist", "duration": 36.0},
    {"id": "sec2", "file": S2_FILE, "name": "Desert Dunes under Full Moon", "duration": 48.0},
    {"id": "sec3", "file": S3_FILE, "name": "Taif Mountain Terraces", "duration": 52.0},
    {"id": "sec4", "file": S4_FILE, "name": "Courtyard Peacock in Garden", "duration": 44.0},
    {"id": "sec5", "file": S5_FILE, "name": "Sunrise over Ocean Shoreline", "duration": 40.0},
]
...
def assemble_master(sections):
    ...
    print("=" * 70)
    print(f"HUURS STUDIO - ASSEMBLING EPISODE 15 FULL MASTER ({total_dur:.2f}s)")
    print("=" * 70)
    ...
    print(f"✓ EPISODE 15 FULL MASTER COMPLETED in {time.time()-t0:.2f}s!")
...
def assemble_short(sections):
    ...
    dur = sec1["duration"]
    print("-" * 70)
    print(f"Assembling 9:16 Vertical Cut (Short A: The Wound of Sharp Words, {dur:.2f}s)...")
    ...
    if res.returncode != 0:
        ...
        sys.exit(1)
    print(f"✓ EPISODE 15 SHORT 001A COMPLETED!")
