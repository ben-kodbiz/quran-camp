#!/usr/bin/env python3
"""
HUURS STUDIO - Episode 20 Full Master Assembly (Season 2 Finale)
================================================================
Assembles all 5 living-creation scene clips for Episode 20 (Surah Ta-Ha 20:2–3, 25–28, 44)
...
- Section 1: Rain on Window & Misty Pine Forest (The Exhaustion of Performance)
- Section 2: Desert Dunes Under Full Moon & Stars (The Revelation That Does Not Crush)
- Section 3: Hummingbird Hovering at Forest Blossom (The Request for an Expanded Chest)
- Section 4: Regal Peacock with Iridescent Plumage (Gentleness in the Face of Arrogance)
- Section 5: The Meeting of Two Seas / Majma' al-Bahrayn (The Eternal Refuge)
...
AUDIO_IN = os.path.join(ROOT_DIR, "11_AUDIO/EP20_audio.mp3")
TIMINGS_JSON = os.path.join(ROOT_DIR, "11_AUDIO/EP20_audio_timings.json")
OUTPUT_MASTER = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP20-FULL-MASTER.mp4")
OUTPUT_SHORT = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP20-SHORT-001A.mp4")
TEMP_DIR = "/tmp/ep20_assembly"

os.makedirs(TEMP_DIR, exist_ok=True)

S1_FILE = os.path.join(RENDERS_DIR, "EP20_SCENE01.mp4")
S2_FILE = os.path.join(RENDERS_DIR, "EP20_SCENE02.mp4")
S3_FILE = os.path.join(RENDERS_DIR, "EP20_SCENE03.mp4")
S4_FILE = os.path.join(RENDERS_DIR, "EP20_SCENE04.mp4")
S5_FILE = os.path.join(RENDERS_DIR, "EP20_SCENE05.mp4")

DEFAULT_SECTIONS = [
    {"id": "sec1", "file": S1_FILE, "name": "Rain on Window & Misty Pines", "duration": 40.0},
    {"id": "sec2", "file": S2_FILE, "name": "Desert Dunes Under Full Moon", "duration": 48.0},
    {"id": "sec3", "file": S3_FILE, "name": "Hummingbird Hovering at Blossom", "duration": 52.0},
    {"id": "sec4", "file": S4_FILE, "name": "Regal Peacock with Plumage", "duration": 44.0},
    {"id": "sec5", "file": S5_FILE, "name": "The Meeting of Two Seas", "duration": 40.0},
]
...
def assemble_master(sections):
    ...
    print("=" * 70)
    print(f"HUURS STUDIO - ASSEMBLING EPISODE 20 FULL MASTER ({total_dur:.2f}s)")
    print("=" * 70)
    ...
    print(f"✓ EPISODE 20 FULL MASTER COMPLETED in {time.time()-t0:.2f}s!")
...
def assemble_short(sections):
    ...
    dur = sec1["duration"]
    print("-" * 70)
    print(f"Assembling 9:16 Vertical Cut (Short A: The Exhaustion of Performance, {dur:.2f}s)...")
    ...
    if res.returncode != 0:
        ...
        sys.exit(1)
    print(f"✓ EPISODE 20 SHORT 001A COMPLETED!")
