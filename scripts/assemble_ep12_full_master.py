#!/usr/bin/env python3
"""
HUURS STUDIO - Episode 12 Full Master Assembly (Season 2)
=========================================================
Assembles all 5 living-creation scene clips for Episode 12 (Surah Yusuf 12:86)
...
- Section 1: Deep Ocean Depths & Synchronized Fish School (The Depth of Unseen Grief)
- Section 2: Infinite Spiral Galaxy & Glowing Nebulae (The Cry of Ya'qub & Celestial Scale)
- Section 3: Vibrant Living Coral Reef & Tropical Fish (The Subtle Decree - Al-Latif)
- Section 4: Bird of Paradise in Misty Rainforest Canopy (Beautiful Patience - Sabrun Jamil)
- Section 5: Emerald Mountain Panorama & Cascading Waterfalls (The Private Grievance Protocol)
...
AUDIO_IN = os.path.join(ROOT_DIR, "11_AUDIO/EP12_audio.mp3")
TIMINGS_JSON = os.path.join(ROOT_DIR, "11_AUDIO/EP12_audio_timings.json")
OUTPUT_MASTER = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP12-FULL-MASTER.mp4")
OUTPUT_SHORT = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP12-SHORT-001A.mp4")
TEMP_DIR = "/tmp/ep12_assembly"

os.makedirs(TEMP_DIR, exist_ok=True)

S1_FILE = os.path.join(RENDERS_DIR, "EP12_SCENE01.mp4")
S2_FILE = os.path.join(RENDERS_DIR, "EP12_SCENE02.mp4")
S3_FILE = os.path.join(RENDERS_DIR, "EP12_SCENE03.mp4")
S4_FILE = os.path.join(RENDERS_DIR, "EP12_SCENE04.mp4")
S5_FILE = os.path.join(RENDERS_DIR, "EP12_SCENE05.mp4")

# Default fallback durations if json not found
DEFAULT_SECTIONS = [
    {"id": "sec1", "file": S1_FILE, "name": "Deep Ocean Depths & Fish School", "duration": 35.0},
    {"id": "sec2", "file": S2_FILE, "name": "Infinite Spiral Galaxy & Nebulae", "duration": 50.0},
    {"id": "sec3", "file": S3_FILE, "name": "Living Coral Reef & Marine Life", "duration": 65.0},
    {"id": "sec4", "file": S4_FILE, "name": "Bird of Paradise in Misty Canopy", "duration": 48.0},
    {"id": "sec5", "file": S5_FILE, "name": "Mountain Panorama & Waterfalls", "duration": 42.0},
]
...
def assemble_master(sections):
    ...
    print("=" * 70)
    print(f"HUURS STUDIO - ASSEMBLING EPISODE 12 FULL MASTER ({total_dur:.2f}s)")
    print("=" * 70)
    ...
    print(f"✓ EPISODE 12 FULL MASTER COMPLETED in {time.time()-t0:.2f}s!")
...
def assemble_short(sections):
    ...
    dur = sec1["duration"]
    print("-" * 70)
    print(f"Assembling 9:16 Vertical Cut (Short A: The Depth of Unseen Grief, {dur:.2f}s)...")
    ...
    if res.returncode != 0:
        ...
        sys.exit(1)
    print(f"✓ EPISODE 12 SHORT 001A COMPLETED!")
