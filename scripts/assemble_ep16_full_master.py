#!/usr/bin/env python3
"""
HUURS STUDIO - Episode 16 Full Master Assembly (Season 2)
=========================================================
Assembles all 5 living-creation scene clips for Episode 16 (Surah An-Nahl 16:18, 68–69, 90)
...
- Section 1: Sunlight through Forest Canopy (The Blindness of Routine)
- Section 2: Living Coral Reef Underwater (The Surah of Blessings)
- Section 3: Honeybees on Golden Honeycomb in Hollow Tree (The Architecture of the Bee)
- Section 4: Crystalline Mountain Stream over River Stones (The Uncountable Currency)
- Section 5: Sacred Sanctuary Courtyard at Sunrise / Masjid al-Haram (The Natural Sight Walk)
...
AUDIO_IN = os.path.join(ROOT_DIR, "11_AUDIO/EP16_audio.mp3")
TIMINGS_JSON = os.path.join(ROOT_DIR, "11_AUDIO/EP16_audio_timings.json")
OUTPUT_MASTER = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP16-FULL-MASTER.mp4")
OUTPUT_SHORT = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP16-SHORT-001A.mp4")
TEMP_DIR = "/tmp/ep16_assembly"

os.makedirs(TEMP_DIR, exist_ok=True)

S1_FILE = os.path.join(RENDERS_DIR, "EP16_SCENE01.mp4")
S2_FILE = os.path.join(RENDERS_DIR, "EP16_SCENE02.mp4")
S3_FILE = os.path.join(RENDERS_DIR, "EP16_SCENE03.mp4")
S4_FILE = os.path.join(RENDERS_DIR, "EP16_SCENE04.mp4")
S5_FILE = os.path.join(RENDERS_DIR, "EP16_SCENE05.mp4")

DEFAULT_SECTIONS = [
    {"id": "sec1", "file": S1_FILE, "name": "Sunlight through Forest Canopy", "duration": 40.0},
    {"id": "sec2", "file": S2_FILE, "name": "Living Coral Reef Underwater", "duration": 48.0},
    {"id": "sec3", "file": S3_FILE, "name": "Honeybees on Golden Honeycomb", "duration": 52.0},
    {"id": "sec4", "file": S4_FILE, "name": "Mountain Stream over River Stones", "duration": 44.0},
    {"id": "sec5", "file": S5_FILE, "name": "Sacred Sanctuary Courtyard at Sunrise", "duration": 40.0},
]
...
def assemble_master(sections):
    ...
    print("=" * 70)
    print(f"HUURS STUDIO - ASSEMBLING EPISODE 16 FULL MASTER ({total_dur:.2f}s)")
    print("=" * 70)
    ...
    print(f"✓ EPISODE 16 FULL MASTER COMPLETED in {time.time()-t0:.2f}s!")
...
def assemble_short(sections):
    ...
    dur = sec1["duration"]
    print("-" * 70)
    print(f"Assembling 9:16 Vertical Cut (Short A: The Blindness of Routine, {dur:.2f}s)...")
    ...
    if res.returncode != 0:
        ...
        sys.exit(1)
    print(f"✓ EPISODE 16 SHORT 001A COMPLETED!")
