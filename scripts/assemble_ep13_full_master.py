#!/usr/bin/env python3
"""
HUURS STUDIO - Episode 13 Full Master Assembly (Season 2)
=========================================================
Assembles all 5 living-creation scene clips for Episode 13 (Surah Ar-Ra'd 13:11, 28)
...
- Section 1: Rain on Window Pane & Forest Mist (The Paradox of Modern Comfort)
- Section 2: Mountain Stream over River Stones (The Ayah of Itmi'nan)
- Section 3: The Meeting of Two Seas (Classical Tafsir & Unshakable Equilibrium)
- Section 4: Noble Arabian Horse in Palm Oasis (Internal Transformation & Grounded Contentment)
- Section 5: Billowing White Clouds Across Blue Sky (The 100-Tasbih Protocol & Expansive Sky)
...
AUDIO_IN = os.path.join(ROOT_DIR, "11_AUDIO/EP13_audio.mp3")
TIMINGS_JSON = os.path.join(ROOT_DIR, "11_AUDIO/EP13_audio_timings.json")
OUTPUT_MASTER = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP13-FULL-MASTER.mp4")
OUTPUT_SHORT = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP13-SHORT-001A.mp4")
TEMP_DIR = "/tmp/ep13_assembly"

os.makedirs(TEMP_DIR, exist_ok=True)

S1_FILE = os.path.join(RENDERS_DIR, "EP13_SCENE01.mp4")
S2_FILE = os.path.join(RENDERS_DIR, "EP13_SCENE02.mp4")
S3_FILE = os.path.join(RENDERS_DIR, "EP13_SCENE03.mp4")
S4_FILE = os.path.join(RENDERS_DIR, "EP13_SCENE04.mp4")
S5_FILE = os.path.join(RENDERS_DIR, "EP13_SCENE05.mp4")

# Default fallback durations if json not found
DEFAULT_SECTIONS = [
    {"id": "sec1", "file": S1_FILE, "name": "Rain on Glass & Forest Mist", "duration": 35.0},
    {"id": "sec2", "file": S2_FILE, "name": "Mountain Stream over Pebbles", "duration": 45.0},
    {"id": "sec3", "file": S3_FILE, "name": "The Meeting of Two Seas", "duration": 55.0},
    {"id": "sec4", "file": S4_FILE, "name": "Arabian Horse in Oasis", "duration": 45.0},
    {"id": "sec5", "file": S5_FILE, "name": "Cumulus Clouds Across Sky", "duration": 40.0},
]
...
def assemble_master(sections):
    ...
    print("=" * 70)
    print(f"HUURS STUDIO - ASSEMBLING EPISODE 13 FULL MASTER ({total_dur:.2f}s)")
    print("=" * 70)
    ...
    print(f"✓ EPISODE 13 FULL MASTER COMPLETED in {time.time()-t0:.2f}s!")
...
def assemble_short(sections):
    ...
    dur = sec1["duration"]
    print("-" * 70)
    print(f"Assembling 9:16 Vertical Cut (Short A: The Paradox of Modern Comfort, {dur:.2f}s)...")
    ...
    if res.returncode != 0:
        ...
        sys.exit(1)
    print(f"✓ EPISODE 13 SHORT 001A COMPLETED!")
