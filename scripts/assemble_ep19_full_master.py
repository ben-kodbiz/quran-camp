#!/usr/bin/env python3
"""
HUURS STUDIO - Episode 19 Full Master Assembly (Season 2)
=========================================================
Assembles all 5 living-creation scene clips for Episode 19 (Surah Maryam 19:4, 9, 23–25)
...
- Section 1: Ocean Dawn with Cool Rolling Sea Mist (The Mathematical Dead End)
- Section 2: Honeybee on Purple Flower in Sunlight (The Secret Whisper & Huwa 'alayya hayyin)
- Section 3: Taif Mountain Agricultural Terraces in Mist (The Withered Palm Trunk)
- Section 4: Shimmering Fish School in Deep Ocean (Unseen Provision in the Deep)
- Section 5: Sunrise Over Ocean Coastline with Coastal Mosque (The Surrender of Maryam)
...
AUDIO_IN = os.path.join(ROOT_DIR, "11_AUDIO/EP19_audio.mp3")
TIMINGS_JSON = os.path.join(ROOT_DIR, "11_AUDIO/EP19_audio_timings.json")
OUTPUT_MASTER = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP19-FULL-MASTER.mp4")
OUTPUT_SHORT = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP19-SHORT-001A.mp4")
TEMP_DIR = "/tmp/ep19_assembly"

os.makedirs(TEMP_DIR, exist_ok=True)

S1_FILE = os.path.join(RENDERS_DIR, "EP19_SCENE01.mp4")
S2_FILE = os.path.join(RENDERS_DIR, "EP19_SCENE02.mp4")
S3_FILE = os.path.join(RENDERS_DIR, "EP19_SCENE03.mp4")
S4_FILE = os.path.join(RENDERS_DIR, "EP19_SCENE04.mp4")
S5_FILE = os.path.join(RENDERS_DIR, "EP19_SCENE05.mp4")

DEFAULT_SECTIONS = [
    {"id": "sec1", "file": S1_FILE, "name": "Ocean Dawn with Sea Mist", "duration": 40.0},
    {"id": "sec2", "file": S2_FILE, "name": "Honeybee on Flower in Sunlight", "duration": 48.0},
    {"id": "sec3", "file": S3_FILE, "name": "Taif Mountain Terraces in Mist", "duration": 52.0},
    {"id": "sec4", "file": S4_FILE, "name": "Deep Ocean Fish School", "duration": 44.0},
    {"id": "sec5", "file": S5_FILE, "name": "Sunrise Over Ocean with Mosque", "duration": 40.0},
]
...
def assemble_master(sections):
    ...
    print("=" * 70)
    print(f"HUURS STUDIO - ASSEMBLING EPISODE 19 FULL MASTER ({total_dur:.2f}s)")
    print("=" * 70)
    ...
    print(f"✓ EPISODE 19 FULL MASTER COMPLETED in {time.time()-t0:.2f}s!")
...
def assemble_short(sections):
    ...
    dur = sec1["duration"]
    print("-" * 70)
    print(f"Assembling 9:16 Vertical Cut (Short A: The Mathematical Dead End, {dur:.2f}s)...")
    ...
    if res.returncode != 0:
        ...
        sys.exit(1)
    print(f"✓ EPISODE 19 SHORT 001A COMPLETED!")
