#!/usr/bin/env python3
"""
HUURS STUDIO - Episode 18 Full Master Assembly (Season 2)
=========================================================
Assembles all 5 living-creation scene clips for Episode 18 (Surah Al-Kahf 18:10, 46)
...
- Section 1: Pine Forest Mist with Sunbeams (The Modern Exile)
- Section 2: Snow-Capped Mountain Peaks Piercing Clouds (The Sanctuary of Mercy)
- Section 3: Ancient Rooted Olive Tree (The Transience of Glitter)
- Section 4: White Birds Soaring Over Waterfall (The Unburdened Flight)
- Section 5: Masjid an-Nabawi Twilight Minarets & Green Dome (The Friday Anchor)
...
AUDIO_IN = os.path.join(ROOT_DIR, "11_AUDIO/EP18_audio.mp3")
TIMINGS_JSON = os.path.join(ROOT_DIR, "11_AUDIO/EP18_audio_timings.json")
OUTPUT_MASTER = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP18-FULL-MASTER.mp4")
OUTPUT_SHORT = os.path.join(RENDERS_DIR, "QURAN-COMEBACK-EP18-SHORT-001A.mp4")
TEMP_DIR = "/tmp/ep18_assembly"

os.makedirs(TEMP_DIR, exist_ok=True)

S1_FILE = os.path.join(RENDERS_DIR, "EP18_SCENE01.mp4")
S2_FILE = os.path.join(RENDERS_DIR, "EP18_SCENE02.mp4")
S3_FILE = os.path.join(RENDERS_DIR, "EP18_SCENE03.mp4")
S4_FILE = os.path.join(RENDERS_DIR, "EP18_SCENE04.mp4")
S5_FILE = os.path.join(RENDERS_DIR, "EP18_SCENE05.mp4")

DEFAULT_SECTIONS = [
    {"id": "sec1", "file": S1_FILE, "name": "Pine Forest Mist with Sunbeams", "duration": 40.0},
    {"id": "sec2", "file": S2_FILE, "name": "Snow-Capped Mountain Peaks", "duration": 48.0},
    {"id": "sec3", "file": S3_FILE, "name": "Ancient Rooted Olive Tree", "duration": 52.0},
    {"id": "sec4", "file": S4_FILE, "name": "White Birds Soaring Over Waterfall", "duration": 44.0},
    {"id": "sec5", "file": S5_FILE, "name": "Masjid an-Nabawi Twilight Minarets", "duration": 40.0},
]
...
def assemble_master(sections):
    ...
    print("=" * 70)
    print(f"HUURS STUDIO - ASSEMBLING EPISODE 18 FULL MASTER ({total_dur:.2f}s)")
    print("=" * 70)
    ...
    print(f"✓ EPISODE 18 FULL MASTER COMPLETED in {time.time()-t0:.2f}s!")
...
def assemble_short(sections):
    ...
    dur = sec1["duration"]
    print("-" * 70)
    print(f"Assembling 9:16 Vertical Cut (Short A: The Modern Exile, {dur:.2f}s)...")
    ...
    if res.returncode != 0:
        ...
        sys.exit(1)
    print(f"✓ EPISODE 18 SHORT 001A COMPLETED!")
