#!/usr/bin/env python3
"""
Huurs Studio - Surah Luqman Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/luqman_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/LUQMAN_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_LUQMAN_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH LUQMAN", w/2 - 115, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE ARCHITECTURE OF WISDOM: PATERNAL MENTORSHIP & THE FIVE KEYS OF THE UNSEEN", w/2 - 315, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of Lahw al-Hadith vs Hikmah, The Filial Debt (Wahnan 'ala Wahn), The Mustard Seed in the Rock, The Triple Mandate of Reform, Somatic Decorum (Sa'ar & Voice Modulation), The Seven Oceans of Ink & Mafatih al-Ghayb", w/2 - 380, 288, font="F1", size=7.1, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("PATERNAL MENTORSHIP & CONSCIENCE", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("HIKMAH, SHUKR & MURAQABAH", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Ya Bunayya Compassion, Shirk as Supreme Injustice", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Mustard Seed in the Boulder & Filial Debt", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("ETHICAL POSTURE & COSMIC UNSEEN", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("SOMATIC DECORUM & MAFATIH AL-GHAYB", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Taming Arrogance, Walking with Poise, Voice Modulation", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Seven Oceans of Ink & The Five Keys of the Unseen", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH LUQMAN MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("WISDOM, SHUKR & INTERNAL CONSCIENCE", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: Book of Wisdom, Luqman's Sagehood, Motherhood & Mustard Seed", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Book of Wisdom & Distraction Trap", "Plate 03", "Ayatu al-Kitabi al-Hakeem, Lahw al-Hadith & Pillars of Heavens"),
    ("Plate 04 : Essence of Wisdom & Monotheism", "Plate 04", "Inna Ash-Shirka La-Zulmun 'Azeem, Gratitude & Consensual Sagehood"),
    ("Plate 05 : Sacred Debt of Motherhood & Limits", "Plate 05", "Wahnan 'ala Wahn, Two Years Weaning & Worldly Accompaniment"),
    ("Plate 06 : The Mustard Seed of Omniscience", "Plate 06", "Habbah min Khardal, Triple Concealment & Living Muraqabah")
]
...
pdf.text("REFORM, CHARACTER & THE FIVE KEYS", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Civic Duty, Somatic Decorum, Seven Oceans & Mafatih al-Ghayb", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Quadruple Pillar of Social Reform", "Plate 07", "Aqimi as-Salah, Amr bil-Ma'roof, Nahy 'an al-Munkar & 'Azm al-Umoor"),
    ("Plate 08 : Somatic Architecture of Character", "Plate 08", "Eradicating Sa'ar Cheek Snobbery, Pacing & Voice Modulation"),
    ("Plate 09 : Seven Oceans of Ink & Creation", "Plate 09", "Limitless Words of Allah, Ka-nafsin Wahidah & Cosmic Signs"),
    ("Plate 10 : Dissolution of Kinship & Five Keys", "Plate 10", "Radical Accountability, Tomorrow's Earnings & Mafatih al-Ghayb"),
...
pdf.text("SURAH LUQMAN FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/luqman_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/luqman_compendium_page-01.png", os.path.join(brain_dir, "luqman_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/luqman_compendium_page-02.png", os.path.join(brain_dir, "luqman_compendium_toc.png"))
