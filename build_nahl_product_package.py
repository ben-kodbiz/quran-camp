#!/usr/bin/env python3
"""
Huurs Studio - Surah An-Nahl Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/nahl_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/NAHL_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_NAHL_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH AN-NAHL", w/2 - 135, 325, font="F2", size=26, rgb=WHITE)
pdf.text("SURAT AL-NI'AM: THE COSMIC ARCHITECTURE OF FAVORS, BIOLOGICAL WISDOM & MORAL CHARTER", w/2 - 345, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of Divine Bounties, Pure Milk, Honey as Healing & The Golden Triad of Ethics Across 5 Foundation Lectures", w/2 - 378, 288, font="F1", size=8.5, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("THE REALM OF BOUNTIES & BIOLOGY", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("AL-NI'AM, MILK & THE INSPIRATION OF THE BEE", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Livestock Splendor (Jamal), Mountain Pegs & Innumerable Favors", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Pure Milk from Chyme & Blood, Architecture of the Bee & Honey Healing", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("MORAL CHARTER & PROPHETIC WISDOM", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("THE GOLDEN TRIAD, WEAVER & DAWAH WITH HIKMAH", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Universal Friday Moral Charter: Justice, Ihsan, Kinship & The Unspooling Yarn", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Hayatan Tayyibah, Duress Exception, Model Ibrahim & The Charter of Dawah", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AN-NAHL MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("COSMIC DECREE, HYDROLOGY & BIOLOGICAL WISDOM", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: Ata Amru Allah, Rains & Oceans, Shirk Fallacy & The Bee's Honey", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Inevitable Decree & The Realm of Livestock", "Plate 03", "Ata Amru Allah, Cosmic Truth, Aesthetic Splendor & Subjugated Mounts"),
    ("Plate 04 : Hydrologic Bounties & The Oceanic Highways", "Plate 04", "Life-giving Rain, Marine Harvest, Stabilizing Mountains & Countless Favors"),
    ("Plate 05 : The Skeptic's Arrogance & Tragedy of Infanticide", "Plate 05", "False Deities, Darkened Faces of Ignorance & Respite of Mercy"),
    ("Plate 06 : Biological Miracle of Milk & The Bee's Honey", "Plate 06", "Pure Milk from Chyme/Blood, Human Ages & The Architect of Honey")
]
...
pdf.text("PARABLES, ETHICS, DURESS & WISDOM IN DAWAH", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Two Slaves Parable, Moral Triad, Duress & Dawah with Hikmah", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Two Parables of Inequality & Domestic Sanctuaries", "Plate 07", "The Helpless vs. Free Benefactor, Sensory Birth & Serene Homes"),
    ("Plate 08 : Golden Triad of Ethics & The Unraveling Weaver", "Plate 08", "Justice, Ihsan, Kinship, The Unspooling Yarn & Hayatan Tayyibah"),
    ("Plate 09 : Reciting Under Sanctuary & The Refuge of Coerced", "Plate 09", "Isti'adhah Protocol, Jibril's Descent & Heart Peaceful with Faith"),
    ("Plate 10 : The Ungrateful City, Model Ibrahim & The Call", "Plate 10", "Hunger and Fear Parable, Ummatan Qanitan & Call with Hikmah"),
...
pdf.text("SURAH AN-NAHL FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/nahl_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/nahl_compendium_page-01.png", os.path.join(brain_dir, "nahl_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/nahl_compendium_page-02.png", os.path.join(brain_dir, "nahl_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/nahl_compendium_page-01.png", f"{PREVIEWS_DIR}/nahl_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/nahl_compendium_page-02.png", f"{PREVIEWS_DIR}/nahl_compendium_toc.png")
