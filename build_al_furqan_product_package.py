#!/usr/bin/env python3
"""
Huurs Studio - Surah Al-Furqan Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/al_furqan_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/AL_FURQAN_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_AL_FURQAN_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH AL-FURQAN", w/2 - 130, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE SOVEREIGN CRITERION, COSMIC SIGNS & 'IBAD AR-RAHMAN", w/2 - 275, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of The Descent of Al-Furqan, The Prophetic Trial, The Regret of Toxic Companionship, The Lengthening Shadow, and The Twelve Hallmarks of Spiritual Nobility", w/2 - 380, 288, font="F1", size=8.0, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("THE CRITERION & THE PROPHETIC TRIAL", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("TABARAKA & THE LIVING BOOK", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Universal Warning, Mortality of Messengers & The Cosmic Fitnah", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Hajr al-Qur'an, Gradual Revelation (Tarteel) & Anchoring the Heart", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("NATURAL MIRACLES & 'IBAD AR-RAHMAN", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("COSMIC SHADOWS & SPIRITUAL ROYALTY", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Madda az-Zill, Purifying Rain, The Marine Barzakh & Human Lineage", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("12 Hallmarks of Nobility, Transmuted Sins & The Chambers of Al-Ghurfah", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AL-FURQAN MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("THE CRITERION, SKEPTICISM & ESCHATOLOGY", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: The Descent of Al-Furqan, Human Prophethood, Biting of Hands & Tarteel", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Divine Criterion & Cosmic Sovereignty", "Plate 03", "Tabaraka, Universal Warning, Absolute Tawheed & Powerless Idols"),
    ("Plate 04 : Prophetic Humanity & The Skeptics' Fallacy", "Plate 04", "Eating Food, Marketplaces, Angered Demands & The Universal Fitnah"),
    ("Plate 05 : Eschatological Reversals & The Day of Regret", "Plate 05", "Scattered Dust (Haba'), Cleaving Skies & The Hands of Remorse"),
    ("Plate 06 : The Abandoned Revelation & Gradual Descent", "Plate 06", "Hajr al-Qur'an, Five Diagnostic Levels & Tarteel Heart-Anchoring")
]
...
pdf.text("HISTORY, COSMOLOGY & 'IBAD AR-RAHMAN", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Ancient Demise, Extending Shadows, Two Waters & Spiritual Nobility", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Historical Precedents & Ego-Deification", "Plate 07", "Musa, Deluge, 'Ad, Thamud, Stone Rain & Deifying Desires (Hawa)"),
    ("Plate 08 : Cosmic Shadows & Hydrological Mercy", "Plate 08", "Madda az-Zill, The Solar Guide, Night Garment & Ma'an Tahura"),
    ("Plate 09 : Maritime Barrier & Biological Genesis", "Plate 09", "Two Waters, The Barzakh Partition, Creation from Water & Nasab/Sihr"),
    ("Plate 10 : 'Ibad ar-Rahman — Spiritual Royalty", "Plate 10", "Serenity (Hawn), Night Vigils, Balance, Transmuted Sins & Al-Ghurfah"),
...
pdf.text("SURAH AL-FURQAN FOUNDATION ARCHITECTURE", w - 245, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/al_furqan_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/al_furqan_compendium_page-01.png", os.path.join(brain_dir, "al_furqan_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/al_furqan_compendium_page-02.png", os.path.join(brain_dir, "al_furqan_compendium_toc.png"))
