#!/usr/bin/env python3
"""
Huurs Studio - Surah Ar-Rum Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/ar_rum_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/AR_RUM_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_AR_RUM_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH AR-RUM", w/2 - 110, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE GEOPOLITICAL PROPHECY: THE COSMIC FITRAH & ECOLOGICAL RETURN", w/2 - 295, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of The Byzantine Prophecy (Fee Bid'i Sineen), The Liturgical Hours of Praise, The Hexad of Cosmic Signs, The Primordial Fitrah, Anthropogenic Corruption (Zahara al-Fasad) & The Revival of Dead Earth Across 4 Movements", w/2 - 380, 288, font="F1", size=7.2, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("GEOPOLITICS & THE COSMIC HEXAD", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("BYZANTINE REVERSAL & SIX SIGNS", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Defeat at Lowest Land, Victory at Badr, Dust Genesis", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Matrimonial Sakīnah, Language Diversity & Cosmic Sleep", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("THE PRIMORDIAL FITRAH & REBIRTH", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("FITRATA ALLAH & ZAHARA AL-FASAD", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Factory Setting of Monotheism, Anti-Sectarian Warning", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Ecological Fasad on Land/Sea & Dead Earth Rain Revival", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AR-RUM MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("GEOPOLITICS, LITURGY & GENESIS", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: Rome's Reversal, Liturgical Hours, Matrimony & Cosmic Sleep", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Geopolitical Prophecy & Sovereignty", "Plate 03", "Ghulibati ar-Room, Lowest Land, Bid'i Sineen & Joy of Badr"),
    ("Plate 04 : Liturgical Praise & Genesis from Dust", "Plate 04", "Five Daily Prayers, Living from Dead & Terrestrial Clay"),
    ("Plate 05 : Matrimonial Sanctuary & Diversity", "Plate 05", "Sign 2: Sakinah, Mawaddah & Rahmah, Sign 3: Tongues & Colors"),
    ("Plate 06 : Sleep, Lightning & Cosmic Summons", "Plate 06", "Circadian Reset, Lightning for Fear & Hope & Resurrection Call")
]
...
pdf.text("FITRAH, ECOLOGY & RESURRECTION", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Primordial Fitrah, Anti-Sectarianism, Zahara al-Fasad & Sabr", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : The Primordial Fitrah & Nature", "Plate 07", "Directing Face to Deen, Fitrata Allah, Pristine Birth & Veils"),
    ("Plate 08 : Sectarianism & Maritime Reality", "Plate 08", "Fragmented Religion, Factions in Dogma, Tempest & Shore Reversion"),
    ("Plate 09 : Anthropogenic Ruin & Awakening", "Plate 09", "Zahara al-Fasad on Land/Sea, Tasting Consequences & Returning"),
    ("Plate 10 : Atmospheric Revival & Seasons of Life", "Plate 10", "Thermodynamic Winds, Rain Analogy, 3 Life Stages & Granite Sabr"),
...
pdf.text("SURAH AR-RUM FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/ar_rum_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/ar_rum_compendium_page-01.png", os.path.join(brain_dir, "ar_rum_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/ar_rum_compendium_page-02.png", os.path.join(brain_dir, "ar_rum_compendium_toc.png"))
