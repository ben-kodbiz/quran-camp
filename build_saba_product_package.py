#!/usr/bin/env python3
"""
Huurs Studio - Surah Saba Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/saba_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/SABA_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_SABA_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH SABA", w/2 - 95, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE DYNASTY OF GRATITUDE, THE COLLAPSE OF SABA & THE SOLITARY REFLECTION", w/2 - 285, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of The Cosmic Hamd, The Omniscience of the Atom's Weight (Mithqala Dharrah), Dawud's Singing Mountains & Pliable Iron, Sulayman's Subjugated Winds & Molten Copper, The Staff of Solomon & The Terminal Termite, The Two Gardens of Ma'rib & The Flood of 'Arim, The Sin of Proximity Fatigue & Scattering of Nations, and The Solitary Intellectual Reflection", w/2 - 380, 288, font="F1", size=7.1, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("THE DYNASTY OF GRATITUDE", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("DAWUD'S IRON & SULAYMAN'S WINDS", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Singing Mountains, Chain Mail & Molten Brass", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Work in Gratitude (I'maloo Ala Dawooda Shukra)", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("THE COLLAPSE OF SABA", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("SAYL AL-'ARIM & OCCULT DEMOLITION", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Burst Dam of Ma'rib, Bitter Scrubland & Scattered Nations", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Wood-Boring Termite Demystifying Jinn Omniscience", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH SABA MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("COSMIC OMNISCIENCE & DYNASTY OF GRATITUDE", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: Cosmic Hamd, Atom's Weight, Dawud's Iron & Sulayman's Winds", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : Cosmic Hamd & The Atom's Weight", "Plate 03", "Fourfold Cosmic Transit, Mithqala Dharrah & Infallible Register"),
    ("Plate 04 : Slander of Delirium & Cosmic Fragility", "Plate 04", "Disintegration Skepticism, Ground-Fissures & Celestial Canopies"),
    ("Plate 05 : Dynasty of Gratitude: Dawud's Iron", "Plate 05", "Singing Mountains, Flocks of Birds, Pliable Iron & Chain Mail"),
    ("Plate 06 : Sulayman's Empire: Winds & Copper", "Plate 06", "Monthly Flights, Fluid Brass Spring, Jinn & I'maloo Shukra")
]
...
pdf.text("OCCULT DEMOLITION, SABA & THE FINAL AWAKENING", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Sulayman's Staff, Flood of 'Arim, Scattering & Solitary Path", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Sulayman's Staff & Occult Demolition", "Plate 07", "Silent Demise in Prayer, The Termite & Jinn Blindness to Ghayb"),
    ("Plate 08 : The Two Gardens & The Burst Dam", "Plate 08", "Baldatun Tayyibah, Sayl al-'Arim, Bitter Scrubland & Recompense"),
    ("Plate 09 : Proximity Fatigue & Scattering of Saba", "Plate 09", "Arrogant Distance Prayer, Dispersal into Folklore & Iblis's Bet"),
    ("Plate 10 : Angelic Awe, Solitude & Final Partition", "Plate 10", "Severing Shirk, Archangels Trembling & Solitary Reflection"),
...
pdf.text("SURAH SABA FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/saba_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/saba_compendium_page-01.png", os.path.join(brain_dir, "saba_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/saba_compendium_page-02.png", os.path.join(brain_dir, "saba_compendium_toc.png"))
