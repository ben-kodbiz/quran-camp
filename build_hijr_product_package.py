#!/usr/bin/env python3
"""
Huurs Studio - Surah Al-Hijr Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/hijr_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/HIJR_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_HIJR_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH AL-HIJR", w/2 - 130, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE FORTRESS OF PRESERVATION, COSMIC DEFENSE & SERENITY IN PROSTRATION", w/2 - 280, 305, font="F2", size=10, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of The Infallible Dhikr, Primordial Genesis, The Immunity of Sincerity & Sujood Across 5 Lectures", w/2 - 365, 288, font="F1", size=8.8, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("PRESERVATION & PRIMORDIAL GENESIS", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("INNA NAHNU NAZZALNA & CLAY", w/2 - box_w - 5, 227, font="F2", size=11, rgb=WHITE)
pdf.text("The Seven-Fold Divine Guarantee of Infallible Scriptural Preservation", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Earthen Sounding Clay (Salsal), Ruh & Universal Angelic Prostration", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("COSMIC CITADEL & THE CURE OF SUJOOD", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("AL-MUKHLISEEN & AL-SAB' AL-MATHANI", w/2 + 25, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Cosmic Defense, Seven Gates of Hell & The Immunity of Sincere Servants", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Geological Warning of Thamud & The Divine Cure for Constriction", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AL-HIJR MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("PRESERVATION, COSMOS, CREATION & PARADISE", c1_x + 12, 365, font="F2", size=9, rgb=CYAN)
pdf.text("Plates 01 to 04: Dhikr Infallibility, Constellations, Earthen Mud & Garden Peace", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : Preservation of Dhikr & Cosmic Protection", "Plate 03", "Inna nahnu nazzalna, Future regret, Burooj & Meteor defense"),
    ("Plate 04 : Primordial Creation & Rebellion of Pride", "Plate 04", "Fertilizing winds, Salsal clay, Ruh infusion & Iblis's refusal"),
    ("Plate 05 : Covenant of Seduction & The Seven Gates", "Plate 05", "Respite granted, Glamorizing evil, Al-Mukhliseen & Hell tiers"),
    ("Plate 06 : Abode of Peace & Balance of Attributes", "Plate 06", "Gardens of springs, Ripped-out rancor & Nabbi' 'ibadee")
]
...
pdf.text("EMISSARIES, CITADELS, MATHANI & SUJOOD", c2_x + 12, 365, font="F2", size=9, rgb=GOLD)
pdf.text("Plates 05 to 08: Ibrahim's Guests, Sodom Overturned, Al-Hijr & The Cure of Sujood", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Angelic Emissaries & Shield Around Lut", "Plate 07", "Guests to Ibrahim, Rebuking despair & Sodom's destruction"),
    ("Plate 08 : Apocalyptic Blast & Citadels of Al-Hijr", "Plate 08", "Overturned city, Sijjeel rain, Thicket & Thamud's failed homes"),
    ("Plate 09 : The Seven Oft-Repeated & Prophetic Solace", "Plate 09", "Bil-Haqq, As-Safh al-Jameel, Al-Fatihah & Lowering the wing"),
    ("Plate 10 : Proclaiming Truth & Medicine of Sujood", "Plate 10", "Fasda' bima tu'mar, Mockers silenced, Tasbih & Sujood"),
...
pdf.text("SURAH AL-HIJR FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/hijr_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/hijr_compendium_page-01.png", os.path.join(brain_dir, "hijr_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/hijr_compendium_page-02.png", os.path.join(brain_dir, "hijr_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/hijr_compendium_page-01.png", f"{PREVIEWS_DIR}/hijr_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/hijr_compendium_page-02.png", f"{PREVIEWS_DIR}/hijr_compendium_toc.png")
