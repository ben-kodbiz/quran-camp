#!/usr/bin/env python3
"""
Huurs Studio - Surah As-Sajdah Master Mindmap HTML & Markdown Generator
EXPANDED EDITION: 10 Landscape Plates • 20 Thematic Pillars • 80 Detailed Analytical Cards
"""

import os, sys, re, json

BASE_DIR = "/mnt/AI/ag/Campaign/07_MINDMAP"
HTML_OUT = os.path.join(BASE_DIR, "AS_SAJDAH_MASTER_MINDMAP.html")
MD_OUT = os.path.join(BASE_DIR, "AS_SAJDAH_MASTER_MINDMAP.md")
PDF_SRC = os.path.join(BASE_DIR, "build_as_sajdah_mindmap_pdf.py")
...
for pidx, p_text in enumerate(pages_raw):
    pnum = pidx + 1
    chrome = re.search(r'draw_chrome\(\s*' + str(pnum) + r',\s*10,\s*"([^"]+)",\s*"([^"]+)",\s*"([^"]+)"\s*\)', p_text)
    if not chrome:
...
# Generate Markdown
md_lines = [
    "# Surah As-Sajdah: Master Mindmap Knowledge Architecture",
    "### EXPANDED JUMU'AH EDITION • 10 Landscape Plates • 20 Thematic Pillars • 80 Analytical Cards",
    "**Source Authority:** Classical Sunni Exegesis (Tabari, Ibn Kathir, Qurtubi, Razi, Baghawi, Ibn al-Qayyim)",
    "**Liturgical Centrality:** Prophetic Friday Dawn Recitation (Sahih al-Bukhari 891; Sahih Muslim 880)",
...
print(f"MD generated: {MD_OUT} ({len(pages_data)} plates, 80 cards)")
...
<head>
  ...
  <title>Surah As-Sajdah Master Mindmap • Huurs Studio</title>
  <style>
    :root {{
      --navy-deep: #060A12;
      --navy-card: #0D1422;
...
<body>
  <div class="wrapper">
    <header class="header">
      <h1>HUURS STUDIO</h1>
      <span>Surah As-Sajdah Master Mindmap • Expanded Jumu'ah Edition (10 Plates • 20 Pillars • 80 Cards)</span>
    </header>
    ...
    <footer class="footer">
      <p>Huurs Knowledge Systems • Authentic Sunni Source Discipline • Read. Reflect. Return.</p>
    </footer>
  </div>
