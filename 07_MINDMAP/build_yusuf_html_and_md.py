#!/usr/bin/env python3
"""
Huurs Studio - Surah Yusuf Master Mindmap HTML & Markdown Generator
...
HTML_OUT = os.path.join(BASE_DIR, "YUSUF_MASTER_MINDMAP.html")
MD_OUT = os.path.join(BASE_DIR, "YUSUF_MASTER_MINDMAP.md")
PDF_SRC = os.path.join(BASE_DIR, "build_yusuf_mindmap_pdf.py")
...
# Generate Markdown
md_lines = [
    "# Surah Yusuf — Master Landscape Mindmap Documentation",
    "## 8 Landscape Plates • 16 Thematic Pillars • 64 Detailed Analytical Cards",
    "",
...
for p in pages_data:
    ...
    html_plates += f"""
    <div class="page-container {active}" id="page-{p['page']}">
      <div class="page-meta">
        <div>
          <h2>{p['title']}</h2>
...
<head>
  ...
  <title>Surah Yusuf — Master Mindmap Cartography | Huurs Studio</title>
  <style>
    :root {{
      --navy-deep: #060a12;
      --navy-card: #0d1422;
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .header span {{ color: var(--text-muted); font-size: 0.85rem; margin-left: 12px; }}
    .tabs-bar {{
      display: flex;
      gap: 8px;
      margin-bottom: 24px;
    ...
    .tab-btn.active {{
      ...
      box-shadow: 0 0 10px rgba(212, 175, 89, 0.2);
    }}
    ...
    .page-meta h2 {{ font-size: 1.2rem; color: var(--gold-light); margin-bottom: 4px; }}
    .page-meta p.desc {{ font-size: 0.85rem; color: var(--text-muted); }}
    .badge-tag {{
    ...
    .pillar-header h3 {{ font-size: 0.95rem; color: var(--white); margin-bottom: 2px; }}
    .pillar-header p {{ font-size: 0.8rem; color: var(--text-muted); }}
    .cards-list {{
      padding: 16px;
      display: flex;
...
<body>
  <div class="wrapper">
    <header class="header">
      <div>
        <h1>HUURS STUDIO</h1>
        <span>Surah Yusuf Master Landscape Mindmap Cartography</span>
      </div>
