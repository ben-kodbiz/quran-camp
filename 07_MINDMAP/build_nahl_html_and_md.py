#!/usr/bin/env python3
"""
Huurs Studio - Surah An-Nahl Master Mindmap HTML & Markdown Generator
...
HTML_OUT = os.path.join(BASE_DIR, "NAHL_MASTER_MINDMAP.html")
MD_OUT = os.path.join(BASE_DIR, "NAHL_MASTER_MINDMAP.md")
PDF_SRC = os.path.join(BASE_DIR, "build_nahl_mindmap_pdf.py")
    ...
    p_obj = {
        ...
        "pillars": [
            ...
            },
            {
                ...
                "color": color_map.get(col2_m.group(2) if col2_m else "GOLD", "gold"),
                "cards": col2_cards
            }
        ]
    }
...
# Generate Markdown
md_lines = [
    "# Surah An-Nahl — Master Landscape Mindmap Documentation",
    "## 8 Landscape Plates • 16 Thematic Pillars • 64 Detailed Analytical Cards",
    "",
...
print(f"MD generated: {MD_OUT} (8 plates, 64 cards)")
...
for p in pages_data:
    ...
    tabs_html += f"""<button class="tab-btn {active}" onclick="showPage({p['page']})">Plate {p['page']:02d}: {p['sec'].split(':')[-1].strip()}</button>\n"""
...
<head>
  ...
  <title>Surah An-Nahl Master Landscape Mindmap | Huurs Studio</title>
  <style>
    :root {{
      --navy-deep: #060a12;
      --navy-card: #0d1422;
...
<body>
  <div class="wrapper">
    <header class="header">
      <div>
        <h1>HUURS STUDIO</h1>
        <span>Surah An-Nahl Master Landscape Mindmap Cartography</span>
      </div>
