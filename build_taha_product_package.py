#!/usr/bin/env python3
"""
Huurs Studio - Surah Ta-Ha Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/taha_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/TAHA_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_TAHA_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH TA-HA", w/2 - 110, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE SOLACE OF REVELATION, THE SACRED FIRE & PROPHETIC EXPANSIVENESS", w/2 - 300, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of The Solace of the Qur'an, The Valley of Tuwa, Musa Before Pharaoh, and Adam's Covenant Across 4 Foundation Lectures", w/2 - 382, 288, font="F1", size=8.5, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("DIVINE SOLACE & THE SACRED VALLEY", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("THE FIRE OF TUWA & PROPHETIC SIGNS", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("The Antidote to Distress, Sacred Grounds & The First Pillar of Prayer", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Staff Transformed, Radiant White Hand & Fraternal Partnership with Harun", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("CONFRONTING TYRANNY & SURRENDER OF TRUTH", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("THE DRY SEA HIGHWAY & ANNIHILATION OF IDOLS", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Mild Speech to Pharaoh, Transcendent Accompaniment & Devouring Staff", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Defiance of Crucifixion, The Golden Calf Destroyed & Shield Against Misery", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH TA-HA MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("SOLACE, THE SACRED VALLEY & PROVIDENCE", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: Antidote to Distress, Fire at Tuwa, Taboot & Dialectic", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Solace of Revelation & Sacred Fire", "Plate 03", "Antidote to Misery, Throne Majesty, Sinai Fire & Removal of Sandals"),
    ("Plate 04 : Celestial Signs & Du'a of Expansiveness", "Plate 04", "Staff Serpent, Radiant Hand, Sharh as-Sadr & Harun's Alliance"),
    ("Plate 05 : Divine Providence & Tapestry of Destiny", "Plate 05", "Floating Taboot, Sister's Vigil, Madyan Exile & Decreed Arrival"),
    ("Plate 06 : Dialectic Before Pharaoh & Gentle Speech", "Plate 06", "Qawlan Layyina, Divine Accompaniment & Definition of Creator")
]
...
pdf.text("CONTEST OF ILLUSION, EXODUS & COVENANT", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Festival Day, Sea Highway, Golden Calf & Adam's Covenant", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : The Contest of Illusion & Triumph of Truth", "Plate 07", "Festival Day, Sensory Magic, Devouring Staff & Sorcerers' Defiance"),
    ("Plate 08 : The Red Sea Highway & The Golden Calf", "Plate 08", "Dry Sea Highway, Drowning Tyrant, Samiri's Calf & Harun's Plea"),
    ("Plate 09 : Indignation, Annihilation & Samiri's Exile", "Plate 09", "Musa's Wrath, Ya Ibna Umma, Pulverizing the Calf & La Misas"),
    ("Plate 10 : The Blind Resurrection & Adam's Covenant", "Plate 10", "Leveled Earth, Constricted Living, Adam's Forgetting & Divine Shield"),
...
pdf.text("SURAH TA-HA FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/taha_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/taha_compendium_page-01.png", os.path.join(brain_dir, "taha_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/taha_compendium_page-02.png", os.path.join(brain_dir, "taha_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/taha_compendium_page-01.png", f"{PREVIEWS_DIR}/taha_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/taha_compendium_page-02.png", f"{PREVIEWS_DIR}/taha_compendium_toc.png")
