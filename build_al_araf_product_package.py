#!/usr/bin/env python3
"""
Huurs Studio - Surah Al-A'raf Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/al_araf_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/AL_ARAF_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_AL_ARAF_MASTER_COMPENDIUM.pdf")
...
# Golden Title Header
pdf.text("SURAH AL-A'RAF", w/2 - 150, 345, font="F2", size=32, rgb=GOLD)
...
# Subtitle
pdf.text("The Definitive Visual Knowledge Architecture of Primordial Memory, Prophetic Epics & The Heights Across Classical Sunni Exegetical Foundations", w/2 - 315, 288, font="F1", size=9.5, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("PRIMORDIAL GENESIS & THE HEIGHTS", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("ADAM, IBLIS & ASHAB AL-A'RAF", w/2 - box_w - 5, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Arrogance of Fire vs. Rabbana Zalamna & Libas at-Taqwa", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Men on the Ramparts & Cleansing Malice from the Chest", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=ROSE, line_width=1.0)
pdf.text("PROPHETIC CRUCIBLES & THE MITHAQ", w/2 + 25, 243, font="F2", size=9.5, rgb=ROSE)
pdf.text("MUSA AT SINAI & ALASTU BI-RABBIKUM", w/2 + 25, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Pulverized Mountain, Sacred Tablets & Seduction of the Calf", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Pre-Temporal Covenant & The Ninety-Nine Beautiful Names", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AL-A'RAF MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("GENESIS, MODESTY, ESCHATOLOGY & EARLY PROPHETS", c1_x + 12, 365, font="F2", size=9, rgb=CYAN)
pdf.text("Plates 01 to 04: Adam & Iblis, Libas at-Taqwa, Ashab al-A'raf, Creation & Nuh/Hud", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : Primordial Genesis & Disobedience", "Plate 03", "Adam & Iblis, Fire vs clay, Sirat ambush, Seduction of tree"),
    ("Plate 04 : Mortal Clothing & Sacred Decorum", "Plate 04", "Rabbana zalamna, Libas at-Taqwa, Zeenah at masjid, Fixed lifespan"),
    ("Plate 05 : The Dialogue of the Heights & Cleansed Hearts", "Plate 05", "Eye of needle, Cursing sister nations, Ashab al-A'raf, Cry for water"),
    ("Plate 06 : Cosmic Creation & Early Prophetic Warnings", "Plate 06", "Sittati Ayyam, Istawa 'alal-'Arsh, Du'a adab, Nuh & Hud to 'Ad")
]
...
pdf.text("PROPHETIC CRUCIBLES, MUSA & THE PRIMORDIAL COVENANT", c2_x + 12, 365, font="F2", size=9, rgb=GOLD)
pdf.text("Plates 05 to 08: Thamud/Lut/Shu'ayb, Musa vs Fir'awn, Sinai Theophany & Mithaq", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Prophetic Crucibles: Thamud, Lut & Madyan", "Plate 07", "Rock palaces, Naqatullah she-camel, Sodomites, Honest scales"),
    ("Plate 08 : The Epic of Musa & The Fall of Sorcerers", "Plate 08", "Living python, Sorcerers prostrate, Five plagues, Sea crossing"),
    ("Plate 09 : Mount Sinai Theophany & Inscribed Tablets", "Plate 09", "Forty nights, Rabbi arini, Pulverized mountain, Golden calf, Ummi prophet"),
    ("Plate 10 : The Primordial Covenant & Universal Sajdah", "Plate 10", "Twelve springs, Sabbath apes, Mithaq Adam, Panting dog, Final Sajdah"),
...
pdf.text("SURAH AL-A'RAF FOUNDATION ARCHITECTURE", w - 245, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/al_araf_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/al_araf_compendium_page-01.png", os.path.join(brain_dir, "al_araf_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/al_araf_compendium_page-02.png", os.path.join(brain_dir, "al_araf_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/al_araf_compendium_page-01.png", f"{PREVIEWS_DIR}/al_araf_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/al_araf_compendium_page-02.png", f"{PREVIEWS_DIR}/al_araf_compendium_toc.png")
