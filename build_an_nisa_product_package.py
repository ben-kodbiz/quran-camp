#!/usr/bin/env python3
"""
Huurs Studio - Surah An-Nisa Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/an_nisa_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/AN_NISA_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_AN_NISA_MASTER_COMPENDIUM.pdf")
...
# Golden Title Header
pdf.text("SURAH AN-NISA", w/2 - 150, 345, font="F2", size=32, rgb=GOLD)
...
# Subtitle
pdf.text("The Definitive Visual Knowledge Architecture of Social Justice & Equity Across Classical Sunni Exegetical Foundations", w/2 - 280, 288, font="F1", size=9.8, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("THE CHARTER OF VULNERABLE RIGHTS", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("EQUITY FOR ORPHANS & WOMEN", w/2 - box_w - 5, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Ontological Origin (Nafsin Wahidah) & Fixed Fara'id Shares", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Emancipating Widows & Principled Marital Companionship", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=ROSE, line_width=1.0)
pdf.text("THE RULE OF LAW & SACRED TRUSTS", w/2 + 25, 243, font="F2", size=9.5, rgb=ROSE)
pdf.text("ABSOLUTE UNIVERSAL JUSTICE", w/2 + 25, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Tu'mah's Trial: Exonerating Innocent & Rejecting Tribalism", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Qawwameena bil-Qist: Standing Firm Against Self & Kin", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AN-NISA MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("SOCIAL ORDER, MARITAL EQUITY & HUMANITARIAN AID", c1_x + 12, 365, font="F2", size=9, rgb=CYAN)
pdf.text("Plates 01 to 04: Primordial Creation, Family Law, Governance & Defense", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : Primordial Origin & Inheritance Boundaries", "Plate 03", "Nafsin Wahidah, Sacred Arham, Orphan wealth, Fara'id shares"),
    ("Plate 04 : Marital Equity, Economic Contracts & Peace", "Plate 04", "Widow emancipation, 'Ashiroohunna, Qiwamah, Two arbiters"),
    ("Plate 05 : Decalogue of Compassion & Governance", "Plate 05", "Ten rings of Ihsan, Amanat to owners, Ayah 59 obedience hierarchy"),
    ("Plate 06 : Sacred Defense, Oppressed Cries & Decree", "Plate 06", "Rescuing Mustad'afeen, Death in towers, Tadabbur proof")
]
...
pdf.text("RULE OF LAW, PURIFIED FAITH & PROPHETIC CHAIN", c2_x + 12, 365, font="F2", size=9, rgb=GOLD)
pdf.text("Plates 05 to 08: Blood Sanctity, Absolute Justice, Hypocrisy & Isa's Vindication", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Sanctity of Blood, Verification & Prayer", "Plate 07", "Homicide law, Fatabayyanoo duty, Hijrah, Salat al-Khawf shifts"),
    ("Plate 08 : Absolute Justice, Treason & Combating Shirk", "Plate 08", "Tu'mah trial, Najwa ethics, Shirk treason, Qawwameena bil-Qist"),
    ("Plate 09 : Anatomy of Hypocrisy & Isa's Vindication", "Plate 09", "Mudhbdhabeen wavering, Ad-Dark al-Asfal, Wa ma qataloohu, Ascension"),
    ("Plate 10 : The Prophetic Chain, Ghuluww & The Seal", "Plate 10", "Cosmic stream, Takleem Musa, Demolishing Trinity, Kalalah seal"),
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/an_nisa_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/an_nisa_compendium_page-01.png", os.path.join(brain_dir, "an_nisa_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/an_nisa_compendium_page-02.png", os.path.join(brain_dir, "an_nisa_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/an_nisa_compendium_page-01.png", f"{PREVIEWS_DIR}/an_nisa_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/an_nisa_compendium_page-02.png", f"{PREVIEWS_DIR}/an_nisa_compendium_toc.png")
