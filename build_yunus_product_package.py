#!/usr/bin/env python3
"""
Huurs Studio - Surah Yunus Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/yunus_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/YUNUS_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_YUNUS_MASTER_COMPENDIUM.pdf")
...
# Golden Title Header
pdf.text("SURAH YUNUS", w/2 - 140, 345, font="F2", size=32, rgb=GOLD)
...
# Subtitle
pdf.text("The Definitive Visual Knowledge Architecture of Cosmic Chronometry, The Human Paradox & The Repentance of Nations Across Classical Sunni Exegetical Foundations", w/2 - 340, 288, font="F1", size=9.2, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("COSMIC TRUTH & THE BEATIFIC VISION", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("KITABIN HAKEEM & AZ-ZIYADAH", w/2 - box_w - 5, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Solar Radiance (Diya') vs Lunar Luminescence (Noor) & Orbital Time", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Gazing upon the Countenance of Allah & The Abode of Peace (Dar as-Salam)", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=ROSE, line_width=1.0)
pdf.text("PROPHETIC VINDICATION & HISTORIC SIGNS", w/2 + 25, 243, font="F2", size=9.5, rgb=ROSE)
pdf.text("PHARAOH'S PRESERVED BODY & YUNUS'S NATION", w/2 + 25, 227, font="F2", size=11, rgb=WHITE)
pdf.text("The Rejected Confession at the Sea & Pharaoh Preserved as a Sign", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Unprecedented Repentance of Nineveh & The Sovereign Fortress of Tawhid", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH YUNUS MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("COSMOS, HUMAN PARADOX, AZ-ZIYADAH & QUR'ANIC HEALING", c1_x + 12, 365, font="F2", size=9, rgb=CYAN)
pdf.text("Plates 01 to 04: Solar/Lunar Order, Sea Tempest, Beatific Vision & 4fold Healing", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : Cosmic Wisdom, Creation & Signs of Time", "Plate 03", "Six epochs, Throne, Diya' vs Noor, Lunar stations (Manazil)"),
    ("Plate 04 : The Human Psychological Paradox & Parables", "Plate 04", "Supplication in agony, Amnesia, Violent sea storm, Blooming meadow"),
    ("Plate 05 : Eschatological Certainty & The Supreme Vision", "Plate 05", "Al-Husna and Az-Ziyadah (Beholding Allah), Radiant countenances, Idols"),
    ("Plate 06 : Miracle of the Qur'an & Healing of Hearts", "Plate 06", "Single Surah challenge, Maw'idhah, Shifa', Huda, Rahmah, Divine grace")
]
...
pdf.text("AWLIYA'ULLAH, EXODUS, PHARAOH & YUNUS'S NATION", c2_x + 12, 365, font="F2", size=9, rgb=GOLD)
pdf.text("Plates 05 to 08: Peace of Awliya, Nuh/Musa, Pharaoh's Corpse & Sincere Repentance", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : The Allies of Allah & Omniscience", "Plate 07", "Awliya without fear/grief, Glad tidings, Weight of an atom (Mithqala Dharrah)"),
    ("Plate 08 : Prophetic Cycle: Nuh & Musa's Crucible", "Plate 08", "Nuh's Tawakkul, Commissioning Musa, Staff swallowing imperial illusions"),
    ("Plate 09 : Exodus, Pharaoh's Drowning & Preserved Body", "Plate 09", "Homes as Qiblah, Parted Red Sea, Al-Aana? Rejected, Corpse preserved"),
    ("Plate 10 : The Miracle of Yunus's Nation & Sovereignty", "Plate 10", "Nineveh's contrition, Lifting doom, Bounds of free will, Hasbiyallah"),
...
pdf.text("SURAH YUNUS FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/yunus_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/yunus_compendium_page-01.png", os.path.join(brain_dir, "yunus_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/yunus_compendium_page-02.png", os.path.join(brain_dir, "yunus_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/yunus_compendium_page-01.png", f"{PREVIEWS_DIR}/yunus_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/yunus_compendium_page-02.png", f"{PREVIEWS_DIR}/yunus_compendium_toc.png")
