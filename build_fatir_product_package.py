#!/usr/bin/env python3
"""
Huurs Studio - Surah Fatir Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/fatir_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/FATIR_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_FATIR_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH FATIR", w/2 - 100, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE COSMIC ORIGINATOR, MULTI-WINGED ANGELS & THE THREE HEIRS OF THE BOOK", w/2 - 305, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of Fatir as-Samawati wal-Ard, Multi-Winged Celestial Ranks, Sovereign Unwithholdable Mercy, Warning Against Al-Gharoor, The Ascent of Pure Speech Elevated by Righteous Deeds, The Parable of the Two Seas, The Date-Seed Membrane (Qitmeer), Human Faqr vs Divine Ghina, Innama Yakhsha Allaha al-'Ulama', The Three Heirs of the Qur'an & The Immutable Sunnah", w/2 - 380, 288, font="F1", size=7.1, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("THE COSMIC ORIGINATOR", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("ANGELIC WINGS & ASCENDING SPEECH", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Fatir, Gabriel's 600 Wings & Unwithholdable Mercy", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Pure Words Rising on the Wings of Righteous Deeds", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("THE THREE HEIRS & SUNNATULLAH", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("INNAMĀ YAKHSHĀ ALLAHA AL-'ULAMĀ'", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("The Self-Wronger, The Moderate & The Foremost in Good", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Date-Seed Membrane (Qitmeer) & The Immutable Sunnah", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH FATIR MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("ORIGINATOR, ANGELS & ASCENDING DEEDS", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: Cosmic Genesis, Wings, Al-Gharoor, Two Seas & Qitmeer", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Cosmic Originator & Angelic Wings", "Plate 03", "Creation Ex Nihilo, Multi-Winged Emissaries & Sovereign Mercy"),
    ("Plate 04 : The Deceiver & Ascent of Pure Speech", "Plate 04", "Al-Gharoor, Satan's Faction, True Glory & The Wings of Action"),
    ("Plate 05 : Morphogenesis & The Two Seas", "Plate 05", "Dust to Embryo, Preserved Lifespans, Rivers & Saline Oceans"),
    ("Plate 06 : Planetary Time & The Date-Seed Skin", "Plate 06", "Circadian Mechanics, Celestial Orbits, The Qitmeer & Deaf Idols")
]
...
pdf.text("HUMAN FAQR, THE THREE HEIRS & SUNNAH", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Ontological Poverty, Geology, The Three Heirs & Sunnatullah", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Human Faqr vs Divine Self-Sufficiency", "Plate 07", "Ontological Poverty, Species Replacement & Unshared Burdens"),
    ("Plate 08 : Sensory Asymmetries & Rock Strata", "Plate 08", "Blind vs Seeing, Reviving Winds & Judad Beed wa Humr"),
    ("Plate 09 : Reverence of Scholars & The Three Heirs", "Plate 09", "Innamā Yakhshā Allaha, Dhalim, Muqtasid, Sabiqun & 'Adn"),
    ("Plate 10 : The Immutable Sunnah & Respite", "Plate 10", "Screams of Remorse, Evil Plots Rebounding & Appointed Hour"),
...
pdf.text("SURAH FATIR FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/fatir_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/fatir_compendium_page-01.png", os.path.join(brain_dir, "fatir_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/fatir_compendium_page-02.png", os.path.join(brain_dir, "fatir_compendium_toc.png"))
