#!/usr/bin/env python3
"""
Huurs Studio - Surah Al-Anfal Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/al_anfal_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/AL_ANFAL_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_AL_ANFAL_MASTER_COMPENDIUM.pdf")
...
# Golden Title Header
pdf.text("SURAH AL-ANFAL", w/2 - 150, 345, font="F2", size=32, rgb=GOLD)
...
# Subtitle
pdf.text("The Definitive Visual Knowledge Architecture of Divine Victory, Spiritual Causality & The Crucible of Badr Across Classical Sunni Exegetical Foundations", w/2 - 325, 288, font="F1", size=9.5, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("METAPHYSICS OF VICTORY & BADR", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("DIVINE AGENCY & ANGELIC DESCENT", w/2 - box_w - 5, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Subordination of Spoils & The Trembling Heart (Innamal-Mu'minoon)", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("'You Did Not Throw When You Threw' & The Thousand Reinforcements", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=ROSE, line_width=1.0)
pdf.text("STRATEGIC ETHICS & BROTHERHOOD", w/2 + 25, 243, font="F2", size=9.5, rgb=ROSE)
pdf.text("SPIRITUAL COHESION & THE CRITERION", w/2 + 25, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Four Pillars of Victory, The Twin Shields of Immunity (Al-Amanan)", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Deterrence without Aggression, Treaty Ethics & Primacy of Kin", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AL-ANFAL MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("BADR, INTERVENTION, CALL TO LIFE & CRITERION", c1_x + 12, 365, font="F2", size=9, rgb=CYAN)
pdf.text("Plates 01 to 04: Spoils of War, Angelic Descent, Inward Life & The Twin Shields", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Spoils of War & The Eve of Badr", "Plate 03", "Subordination of spoils, Trembling heart, Caravan vs army, 1000 angels"),
    ("Plate 04 : Divine Intervention & The Cast Sand", "Plate 04", "Angelic strikes, Prohibition of retreat, 'Allah threw', Deaf/dumb beasts"),
    ("Plate 05 : The Call to Life & The Plots of Makkah", "Plate 05", "Responding to life, Sovereignty over hearts, Fitnah prevention, Dar an-Nadwah"),
    ("Plate 06 : The Pre-Islamic Mindset & The Criterion", "Plate 06", "The shield of Istighfar, Whistling at sanctuary, Khumus division, Furqan")
]
...
pdf.text("COHESION, TREATIES, DEFENSE & BROTHERHOOD", c2_x + 12, 365, font="F2", size=9, rgb=GOLD)
pdf.text("Plates 05 to 08: Four Pillars of Victory, Satan's Flight, Deterrence & Primacy of Kin", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Geometry of Conflict & Cohesion", "Plate 07", "Topography of Badr, Minimizing dreams, Four pillars of victory, No disputes"),
    ("Plate 08 : Betrayal of Satan & Treaty Ethics", "Plate 08", "Cowardice of Iblis, Changing inward states, Treaty ethics & treason"),
    ("Plate 09 : Readiness for Defense & Peace", "Plate 09", "Preparing maximum deterrence, Incline to peace, Reconciled hearts, Ratios"),
    ("Plate 10 : Captives of War, Brotherhood & Kin", "Plate 10", "Judgment on captives, Halal Tayyib spoils, The pact of Wilayah, Ulul-Arham"),
...
pdf.text("SURAH AL-ANFAL FOUNDATION ARCHITECTURE", w - 245, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/al_anfal_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/al_anfal_compendium_page-01.png", os.path.join(brain_dir, "al_anfal_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/al_anfal_compendium_page-02.png", os.path.join(brain_dir, "al_anfal_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/al_anfal_compendium_page-01.png", f"{PREVIEWS_DIR}/al_anfal_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/al_anfal_compendium_page-02.png", f"{PREVIEWS_DIR}/al_anfal_compendium_toc.png")
