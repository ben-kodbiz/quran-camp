#!/usr/bin/env python3
"""
Huurs Studio - Surah Al-Ma'idah Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/al_maidah_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/AL_MAIDAH_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_AL_MAIDAH_MASTER_COMPENDIUM.pdf")
...
# Golden Title Header
pdf.text("SURAH AL-MA'IDAH", w/2 - 170, 345, font="F2", size=32, rgb=GOLD)
...
# Subtitle
pdf.text("The Definitive Visual Knowledge Architecture of Covenants, Sacramental Law & Sovereignty Across Classical Sunni Exegetical Foundations", w/2 - 290, 288, font="F1", size=9.8, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("THE INVIOLABILITY OF COVENANTS", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("SACRED PACTS & PURIFICATION", w/2 - box_w - 5, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Awfoo bil-'Uqud & Perfection of Deen (Al-Yawma Akmaltu)", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Transcendent Equity: I'diloo Huwa Aqrabu lit-Taqwa", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=ROSE, line_width=1.0)
pdf.text("SACRAMENTAL LAW & ESCHATOLOGY", w/2 + 25, 243, font="F2", size=9.5, rgb=ROSE)
pdf.text("SCRIPTURE, REVOLUTION & SEALS", w/2 + 25, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Cosmic Sanctity of Life (Habil/Qabil) & Muhaymin Criterion", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Heavenly Table & Isa's Solemn Defense (Subhanaka)", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text("Tafsir al-Tabari  *  Tafsir Ibn Kathir  *  Tafsir al-Qurtubi  *  Mafatih al-Ghayb (Al-Razi)  *  Ahkam al-Qur'an (Al-Jassas)", w/2 - 275, 70, font="F1", size=8, rgb=TEXT_MUTED)
...
pdf.text(" |  SURAH AL-MA'IDAH MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("COVENANTS, PURITY, LIFE & DIVINE CRITERION", c1_x + 12, 365, font="F2", size=9, rgb=CYAN)
pdf.text("Plates 01 to 04: Master Pacts, Habil/Qabil, Penal Law & Revelation", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : Inviolability of Covenants & Boundaries", "Plate 03", "Awfoo bil-'Uqud, Completion of Deen, Taharah, Objective Justice"),
    ("Plate 04 : Historical Covenants & Sanctity of Life", "Plate 04", "Twelve Chieftains, Wilderness cowardice, Habil/Qabil, Saving life"),
    ("Plate 05 : Social Order, Penal Justice & Devotion", "Plate 05", "Hirabah banditry, Seeking Al-Wasilah, Protecting wealth, Equity"),
    ("Plate 06 : Progression of Scripture & The Guardian", "Plate 06", "Torah, Injeel, The Qur'an as Muhaymin, Shir'atan wa Minhaja")
]
...
pdf.text("ALLIANCE, PURITY, CIVIC SANCTUARY & SEALS", c2_x + 12, 365, font="F2", size=9, rgb=GOLD)
pdf.text("Plates 05 to 08: Wilayah, Rebutting Shirk, Ka'bah Purity & Judgment", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Geometry of Alliance & Divine Affection", "Plate 07", "Wilayah, The Loving Vanguard (Yuhibbuhum), Bal Yadahu, Balligh"),
    ("Plate 08 : Exposing Theological Excess & Reform", "Plate 08", "Refuting Trinity, Kana Ya'kulani at-Ta'am, Curse on silence, Monks"),
    ("Plate 09 : Social Ethics, Sanctuary Purity & Oaths", "Plate 09", "Oaths expiation, Banning Khamr/Maysir, Ihram tests, Sacred Ka'bah"),
    ("Plate 10 : Eschatological Inquiry & Sovereign Seal", "Plate 10", "Miracles bi-idhnillah, The Heavenly Table, Subhanaka defense"),
    ("Core Architecture : 16 Thematic Pillars", "64 Cards", "Complete Sunni Exegetical Synthesis • 100% Tier-1 Certified"),
    ("Theological Standard : Mutawatir & Classical Sunni", "Tier-1", "Tafsir al-Tabari, Ibn Kathir, Al-Qurtubi, Al-Razi, Ahkam al-Jassas")
]
...
pdf.text("SURAH AL-MA'IDAH FOUNDATION ARCHITECTURE", w - 245, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/al_maidah_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/al_maidah_compendium_page-01.png", os.path.join(brain_dir, "al_maidah_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/al_maidah_compendium_page-02.png", os.path.join(brain_dir, "al_maidah_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/al_maidah_compendium_page-01.png", f"{PREVIEWS_DIR}/al_maidah_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/al_maidah_compendium_page-02.png", f"{PREVIEWS_DIR}/al_maidah_compendium_toc.png")
