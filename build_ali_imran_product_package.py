#!/usr/bin/env python3
"""
Huurs Studio - Surah Ali-Imran Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/ali_imran_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/ALI_IMRAN_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_ALI_IMRAN_MASTER_COMPENDIUM.pdf")
...
# Golden Title Header
pdf.text("SURAH ALI-IMRAN", w/2 - 170, 345, font="F2", size=32, rgb=GOLD)
...
# Subtitle
pdf.text("The Definitive Visual Knowledge Architecture of the Family of Imran Across Classical Sunni Exegetical Foundations", w/2 - 275, 288, font="F1", size=10, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("THE THEOLOGICAL DIALOGUE (9 AH)", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("CHRISTOLOGICAL MONOTHEISM", w/2 - box_w - 5, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Kamathali Adam: Creation from Dust & The Mubahalah Proof", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Rooted Scholars & Epistemological Humility in Revelation", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=ROSE, line_width=1.0)
pdf.text("THE CRUCIBLE OF UHUD (3 AH)", w/2 + 25, 243, font="F2", size=9.5, rgb=ROSE)
pdf.text("BATTLEFIELD AUDIT & RESTORATION", w/2 + 25, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Mount Rumah, Archers' Deviation & Sorrow Upon Sorrow", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Prophetic Mildness (Linta Lahum) & The Living Martyrs", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text("Tafsir al-Tabari  *  Tafsir Ibn Kathir  *  Tafsir al-Qurtubi  *  Mafatih al-Ghayb (Al-Razi)  *  Zad al-Ma'ad (Ibn al-Qayyim)", w/2 - 275, 70, font="F1", size=8, rgb=TEXT_MUTED)
...
pdf.text(" |  SURAH ALI-IMRAN MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("THEOLOGY, CREED & SCRIPTURAL DIALOGUE", c1_x + 12, 365, font="F2", size=9, rgb=CYAN)
pdf.text("Plates 01 to 04: Epistemology, Imran's Lineage, Isa & The Patriarch", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : Epistemology, Revelation & Primordial Creed", "Plate 03", "Muhkam/Mutashabih, Rooted scholars, Rabbana la tuzigh, Love metric"),
    ("Plate 04 : The Family of Imran & Maryam's Sanctuary", "Plate 04", "Hannah's consecrated vow, Mihrab miracles, Zakariyya & Yahya"),
    ("Plate 05 : Reality of Isa & The Decisive Proof", "Plate 05", "Creation from dust (Kamathali Adam), Miracles, Mubahalah"),
    ("Plate 06 : Scriptural Dialogue & Ibrahimic Standard", "Plate 06", "Kalimatin Sawa', Hanifan Musliman, Mithaq al-Nabiyyeen, Peak charity")
]
...
pdf.text("CRISIS, COMMUNITY & THE COSMIC SEAL", c2_x + 12, 365, font="F2", size=9, rgb=GOLD)
pdf.text("Plates 05 to 08: Charter of Unity, Uhud, Governance & Cosmological Seal", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : The Golden Anchor of Unity & Best Nation", "Plate 07", "Hablillah cable, Aws & Khazraj reconciliation, Khayra Ummah mandate"),
    ("Plate 08 : The Crucible of Uhud: Discipline & Audit", "Plate 08", "Mount Rumah archers, Badr contrast, sorrow upon sorrow, motive audit"),
    ("Plate 09 : Leadership in Crisis & The Living Martyrs", "Plate 09", "Linta lahum mildness, Shura restoration, Tawakkul, emerald birds"),
    ("Plate 10 : The Ulul-Albab & The Cosmological Seal", "Plate 10", "Cosmic contemplation, purpose of creation, quadruple success mandate"),
    ("Core Architecture : 16 Thematic Pillars", "64 Cards", "Complete Sunni Exegetical Synthesis • 100% Tier-1 Certified"),
    ("Theological Standard : Mutawatir & Classical Sunni", "Tier-1", "Tafsir al-Tabari, Ibn Kathir, Al-Qurtubi, Al-Razi, Zad al-Ma'ad")
]
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/ali_imran_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/ali_imran_compendium_page-01.png", os.path.join(brain_dir, "ali_imran_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/ali_imran_compendium_page-02.png", os.path.join(brain_dir, "ali_imran_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/ali_imran_compendium_page-01.png", f"{PREVIEWS_DIR}/ali_imran_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/ali_imran_compendium_page-02.png", f"{PREVIEWS_DIR}/ali_imran_compendium_toc.png")
