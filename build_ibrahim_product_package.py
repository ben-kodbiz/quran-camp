#!/usr/bin/env python3
"""
Huurs Studio - Surah Ibrahim Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/ibrahim_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/IBRAHIM_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_IBRAHIM_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH IBRAHIM", w/2 - 135, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE ARCHITECTURE OF LIGHT, THE PURE TREE & ABRAHAMIC GRATITUDE", w/2 - 265, 305, font="F2", size=10.5, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of Light from Darkness, The Pure Tree, Satan's Confession & Abrahamic Foundations Across 4 Lectures", w/2 - 365, 288, font="F1", size=9, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("LIGHT FROM DARKNESS & SHUKR", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("AYYAM ALLAH & LA'IN SHAKARTUM", w/2 - box_w - 5, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Extraction from Plural Darknesses (Az-Zulumat) to the Singular Light", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Universal Law of Gratitude & Deeds Blown Like Stormy Ashes", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("THE PURE TREE & ABRAHAMIC DU'A", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("KALIMAH TAYYIBAH & MAKKAH", w/2 + 25, 227, font="F2", size=11, rgb=WHITE)
pdf.text("The Pure Tree Anchored to Bedrock & Satan's Grand Khutbah in Hell", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Ibrahim at the Uncultivated Valley, Prayer & The Transmuted Earth", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH IBRAHIM MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("REVELATION, GRATITUDE, RESISTANCE & THE PURE TREE", c1_x + 12, 365, font="F2", size=9, rgb=CYAN)
pdf.text("Plates 01 to 04: Zulumat to Noor, Ayyam Allah, Stormy Ashes & Kalimah Tayyibah", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : Darkness to Light & Vernacular", "Plate 03", "Zulumat to Noor, Language of messengers, Ayyam Allah & Musa"),
    ("Plate 04 : Equation of Gratitude & Stormy Ashes", "Plate 04", "La'in shakartum, God's independence, Skepticism & Blown ashes"),
    ("Plate 05 : Tyrants' Exile & Festering Sadeed", "Plate 05", "Threats of expulsion, Divine victory & Agonizing festering drinks"),
    ("Plate 06 : Khutbah of Iblis & The Pure Tree", "Plate 06", "Satan's confession in Fire, Blame yourselves & Kalimah Tayyibah")
]
...
pdf.text("DIVINE ANCHORING, ABRAHAMIC FOUNDATIONS & JUDGMENT", c2_x + 12, 365, font="F2", size=9, rgb=GOLD)
pdf.text("Plates 05 to 08: Al-Qawl ath-Thabit, Makkah Valley, Staring Eyes & Transmuted Earth", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Evil Tree & Divine Anchoring", "Plate 07", "Uprooted weed, Al-Qawl ath-Thabit, Trading grace for ruin & Spending"),
    ("Plate 08 : Cosmic Order & The Barren Valley", "Plate 08", "Sun/moon orbits, Countless blessings & Ibrahim settling at Makkah"),
    ("Plate 09 : Abrahamic Prayer & Delayed Reckoning", "Plate 09", "Establishers of prayer, Parental pardon & Staring eyes in terror"),
    ("Plate 10 : Transmuted Earth & Final Proclamation", "Plate 10", "Mountain conspiracies, Replaced earth, Pitch garments & Balagh"),
...
pdf.text("SURAH IBRAHIM FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/ibrahim_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/ibrahim_compendium_page-01.png", os.path.join(brain_dir, "ibrahim_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/ibrahim_compendium_page-02.png", os.path.join(brain_dir, "ibrahim_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/ibrahim_compendium_page-01.png", f"{PREVIEWS_DIR}/ibrahim_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/ibrahim_compendium_page-02.png", f"{PREVIEWS_DIR}/ibrahim_compendium_toc.png")
