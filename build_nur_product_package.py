#!/usr/bin/env python3
"""
Huurs Studio - Surah An-Nur Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/nur_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/NUR_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_NUR_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH AN-NUR", w/2 - 105, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE ARCHITECTURE OF LIGHT, MORAL SANCTITY & DOMESTIC PRIVACY", w/2 - 260, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of The Ifk Ordeal, Domestic Isti'dhan, Ayah an-Nur (Light Upon Light), Parables of Darkness, and Ayat al-Istikhlaf Across 5 Foundation Lectures", w/2 - 380, 288, font="F1", size=8.5, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("MORAL SANCTITY & VINDICATION", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("HADITH AL-IFK & ABU BAKR'S PARDON", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Four Witnesses for Qadhf, Li'an & Presumption of Innocence", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Bal Huwa Khayrun Lakum & The Supreme Pardon of Mistah", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("AYAH AN-NUR & AYAT AL-ISTIKHLAF", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("LIGHT UPON LIGHT & GLOBAL SUCCESSION", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("The Niche, Crystal Glass, Pearlescent Star & Olive Tree", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Houses of Dhikr, Mirage, Abyssal Ocean & The Promise of Istikhlaf", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AN-NUR MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("LEGISLATION, VINDICATION & PRIVACY", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: Judicial Statutes, Hadith al-Ifk, Isti'dhan & Ayah an-Nur", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Legislative Bastion & Sanctity of Lineage", "Plate 03", "Faradhnaha, Four Witnesses for Qadhf, Li'an & Divine Grace"),
    ("Plate 04 : The Ordeal of Slander & Communal Ethics", "Plate 04", "Hadith al-Ifk, Wholesome Presumption, Gossip & Abu Bakr's Pardon"),
    ("Plate 05 : Domestic Privacy & Protocols of Modesty", "Plate 05", "Isti'dhan, Turning Back Gracefully, Lowering Gaze & The Veil"),
    ("Plate 06 : Ayah an-Nur (Light Upon Light)", "Plate 06", "The Niche, Crystal Glass, Pearlescent Star & The Blessed Olive")
]
...
pdf.text("HOUSES OF DHIKR, ISTIKHLAF & DECORUM", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Parables of Darkness, Winged Flocks, Istikhlaf & Prophetic Decorum", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Houses of Light & Parables of Darkness", "Plate 07", "Sanctuaries of Dhikr, Men Undistracted, Desert Mirage & Deep Abyss"),
    ("Plate 08 : Cosmic Liturgy & Meteorological Wonders", "Plate 08", "Birds in Flight, Cloud Stacks (Rukaman), Lightning & Water Matrix"),
    ("Plate 09 : Arbitration of Truth & Ayat al-Istikhlaf", "Plate 09", "Hypocrisy Exposed, Sami'na wa Ata'na & The Charter of Power"),
    ("Plate 10 : Domestic Etiquette & Prophetic Reverence", "Plate 10", "Three Hours of Privacy, Social Dining & Prophetic Assembly"),
...
pdf.text("SURAH AN-NUR FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/nur_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/nur_compendium_page-01.png", os.path.join(brain_dir, "nur_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/nur_compendium_page-02.png", os.path.join(brain_dir, "nur_compendium_toc.png"))
