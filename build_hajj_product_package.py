#!/usr/bin/env python3
"""
Huurs Studio - Surah Al-Hajj Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/hajj_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/HAJJ_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_HAJJ_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH AL-HAJJ", w/2 - 110, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE COSMIC TREMOR, THE ANCIENT SANCTUARY & THE METAPHYSICS OF SACRIFICE", w/2 - 310, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of The Cataclysmic Hour, Biological Signs, Abrahamic Pilgrimage, Multi-Confessional Sanctuaries, and The Parable of the Fly Across 6 Foundation Lectures", w/2 - 380, 288, font="F1", size=8.5, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("COSMIC DREAD & EMBRYOLOGICAL PROOFS", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("THE CATACLYSM & THE SPROUTING SEED", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Zalzalat al-Sa'ah, Maternal Bonds Severed & Intoxicated Terror", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Seven Stages of Gestation, Decrepitude & The Quivering Soil", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("SACRED RITES, DEFENSE & THE PARABLE", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("THE ANCIENT HOUSE & THE FLY'S CRUMB", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Purification of the Ka'bah, Global Call Across Mountain Ravines", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Transcendence of Flesh, Defense of Churches & Mosques, Parable of Fly", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AL-HAJJ MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("ESCHATOLOGY, REASON & THE SACRED HOUSE", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: The Cataclysm, Embryo, The Precipice & Abrahamic Sanctuary", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Cataclysmic Tremor & Proofs of Resurrection", "Plate 03", "Zalzalat al-Sa'ah, Maternal Bonds, Gestation & Quivering Soil"),
    ("Plate 04 : Faith on the Precipice & Celestial Realities", "Plate 04", "'Ibadah 'Ala Harf, Both Worlds Lost, Impotent Gods & Divine Will"),
    ("Plate 05 : The Ancient Sanctuary & Abrahamic Summons", "Plate 05", "Al-Bayt al-'Ateeq, Decontamination, Global Adhan & Deep Ravines"),
    ("Plate 06 : Sacrificial Rites & The Metaphysics of Taqwa", "Plate 06", "Sha'a'ir Allah, Camels in Ranks, Feeding Poor & Meat vs Taqwa")
]
...
pdf.text("DEFENSE, RUINED CITADELS & THE PARABLE", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Charter of Defense, Ruined Towers, Parable of Fly & Muslim Identity", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : The Charter of Defense & Sacred Enclaves", "Plate 07", "Sanction to Fight, Daf'ullah, Monasteries, Synagogues & Mosques"),
    ("Plate 08 : Ruined Citadels & The Blindness of the Heart", "Plate 08", "Ancient Ruins, Dried Wells, Loft Towers & Blindness in Breasts"),
    ("Plate 09 : The Parable of the Fly & Absolute Impotence", "Plate 09", "Mathal adh-Dhubab, Stolen Crumb, Joint Weakness & True Grandeur"),
    ("Plate 10 : The Abrahamic Legacy & The Noble Call", "Plate 10", "Four-Fold Liturgy, No Hardship, The Muslim Identity & Best Helper"),
...
pdf.text("SURAH AL-HAJJ FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/hajj_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/hajj_compendium_page-01.png", os.path.join(brain_dir, "hajj_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/hajj_compendium_page-02.png", os.path.join(brain_dir, "hajj_compendium_toc.png"))
print("Compendium Cover & TOC previews copied to brain directory.")
