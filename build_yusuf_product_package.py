#!/usr/bin/env python3
"""
Huurs Studio - Surah Yusuf Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/yusuf_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/YUSUF_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_YUSUF_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH YUSUF", w/2 - 125, 325, font="F2", size=26, rgb=WHITE)
pdf.text("AHSAN AL-QASAS: THE PSYCHOLOGY OF SABR JAMEEL & THE REALIZATION OF DREAMS", w/2 - 290, 305, font="F2", size=10.5, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of The Celestial Vision, The Seduction Crucible, Prison Da'wah & Cosmic Reconciliation Across Classical Sunni Exegetical Foundations", w/2 - 365, 288, font="F1", size=8.8, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("CHASTITY & THE BURHAN OF THE LORD", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("MA'ADHALLAH & PRISON INTEGRITY", w/2 - box_w - 5, 227, font="F2", size=11, rgb=WHITE)
pdf.text("The Closed Doors (Hayta Lak) & The Forensic Tear From Behind", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Choosing Prison Over Sin (As-Sijnu Ahabbu Ilayya) & Monotheism in Chains", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=ROSE, line_width=1.0)
pdf.text("BEAUTIFUL PATIENCE & CELESTIAL REALIZATION", w/2 + 25, 243, font="F2", size=9.5, rgb=ROSE)
pdf.text("SABRUN JAMEEL & ANA YUSUF", w/2 + 25, 227, font="F2", size=11, rgb=WHITE)
pdf.text("Eyes White with Suppressed Grief (Baththee wa Huznee ila Allah)", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Immortal Pardon (La Tathreeba 'Alaykumul-Yawm) & Prostration of the Stars", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH YUSUF MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("VISION, FRATERNAL ENVY, SEDUCTION & PRISON", c1_x + 12, 365, font="F2", size=9, rgb=CYAN)
pdf.text("Plates 01 to 04: Celestial Dream, Well of Ghayabah, Ma'adhallah & Banquet of Knives", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Celestial Dream & Paternal Wisdom", "Plate 03", "Ahsan al-Qasas, Eleven stars vision, Kitman an-Ni'mah, Chosen lineage"),
    ("Plate 04 : Fraternal Envy, The Well & False Blood", "Plate 04", "Conspiracy, Ghayabatil-jubb, Untorn bloody shirt, Sabrun Jameel"),
    ("Plate 05 : Seduction & The Burhan of the Lord", "Plate 05", "Shatr al-husn, Locked doors (Hayta lak), Ma'adhallah, Qudda min dubur"),
    ("Plate 06 : Banquet of Knives & Choosing Prison", "Plate 06", "Scandal, Sliced hands (Malakun kareem), As-Sijnu ahabbu ilayya, Sacred retreat")
]
...
pdf.text("PRISON DA'WAH, STATECRAFT & CELESTIAL FULFILLMENT", c2_x + 12, 365, font="F2", size=9, rgb=GOLD)
pdf.text("Plates 05 to 08: 14-Year Agrarian Plan, Exoneration, Benjamin's Cup & Grand Reconciliation", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Prison Da'wah & The King's Dream", "Plate 07", "Tawhid khutbah, Seven fat/lean cows, Storing in the ear (Fee sunbulihi)"),
    ("Plate 08 : Exoneration, High Office & Famine Relief", "Plate 08", "Hashasa al-haqq, Innee hafeezun 'aleem, Famine governance, Brothers arrive"),
    ("Plate 09 : Benjamin's Cup & The Agony of Ya'qub", "Plate 09", "Separate gates, Royal cup (Siqayah), Baththee wa huznee ila Allah, White eyes"),
    ("Plate 10 : Grand Reconciliation & Celestial Fulfillment", "Plate 10", "Ana Yusuf, La tathreeb, Shirt of vision, Prostration of stars, Tawaffanee Musliman"),
...
pdf.text("SURAH YUSUF FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/yusuf_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/yusuf_compendium_page-01.png", os.path.join(brain_dir, "yusuf_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/yusuf_compendium_page-02.png", os.path.join(brain_dir, "yusuf_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/yusuf_compendium_page-01.png", f"{PREVIEWS_DIR}/yusuf_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/yusuf_compendium_page-02.png", f"{PREVIEWS_DIR}/yusuf_compendium_toc.png")
