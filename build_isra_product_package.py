#!/usr/bin/env python3
"""
Huurs Studio - Surah Al-Isra Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/isra_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/ISRA_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_ISRA_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH AL-ISRA", w/2 - 135, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE NOCTURNAL ASCENSION, THE MORAL DECALOGUE & THE METAPHYSICS OF HUMAN DIGNITY", w/2 - 340, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of The Night Journey, Prophesied Epochs, Universal Filial Piety & The Praised Station Across 4 Foundation Lectures", w/2 - 382, 288, font="F1", size=8.5, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("CELESTIAL ASCENSION & HISTORICAL LAWS", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("SUBHANA ALLADHI ASRA & CYCLES OF CORRUPTION", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("The Nocturnal Journey to Sacred Al-Aqsa & Divine Servitude ('Abdihi)", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Two Decrees of Transgression, Babylonian Scourge & The Iron Law of History", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("THE UNIVERSAL DECALOGUE & HUMAN DIGNITY", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("BIRR AL-WALIDAYN, WA LAQAD KARRAMNA & TAHAJJUD", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Inviolable Charter of Parental Piety: Banning 'Uff' & Lowering the Wing", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Inherent Human Nobility, The Secret of the Soul & The Praised Station", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AL-ISRA MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("ASCENT, HISTORICAL CYCLES & THE MORAL DECALOGUE", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: Subhan, Bani Isra'il Epochs, Upright Guide & Filial Piety", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Celestial Night Journey & Two Historical Cycles", "Plate 03", "Subhan Alladhi Asra, Bodily Ascent, Al-Aqsa & Babylonian Scourge"),
    ("Plate 04 : Upright Scripture & Personal Moral Liability", "Plate 04", "Lillati Hiya Aqwam, Neck Ledger, Iqra' Kitabak & Zero Collective Guilt"),
    ("Plate 05 : The Ethical Decalogue: Parental Piety & Solidarity", "Plate 05", "Birr al-Walidayn, Ban on 'Uff', Lowered Wing & Balanced Finances"),
    ("Plate 06 : The Ethical Decalogue: Life & Cognitive Truth", "Plate 06", "Banning Child Slaughter, Sadd al-Dhara'i' in Zina & Sensory Audit")
]
...
pdf.text("COSMIC PRAISE, HUMAN DIGNITY, SPIRIT & TEARS", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Universal Tasbih, Inherent Karamah, Tahajjud & Prostrations", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : The Cosmic Choir & Veil Over Skeptic Hearts", "Plate 07", "Universal Atomic Praise, Invisible Barrier & Resurrection Rebuttal"),
    ("Plate 08 : Primordial Dignity of Adam & The Adversarial Vow", "Plate 08", "Wa Laqad Karramna, Satanic Vow to Bridle & Fortress of Servants"),
    ("Plate 09 : The Enigma of the Spirit & The Night Vigil", "Plate 09", "Al-Ruh Min Amr Rabbi, Qur'anic Shifa', Tahajjud & Station of Praise"),
    ("Plate 10 : Nine Proofs, Tears of Humility & Closing Doxology", "Plate 10", "Musa's Clear Signs, Weeping Upon Chins & Closing Takbir"),
...
pdf.text("SURAH AL-ISRA FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/isra_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/isra_compendium_page-01.png", os.path.join(brain_dir, "isra_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/isra_compendium_page-02.png", os.path.join(brain_dir, "isra_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/isra_compendium_page-01.png", f"{PREVIEWS_DIR}/isra_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/isra_compendium_page-02.png", f"{PREVIEWS_DIR}/isra_compendium_toc.png")
