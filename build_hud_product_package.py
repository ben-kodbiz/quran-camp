#!/usr/bin/env python3
"""
Huurs Studio - Surah Hud Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/hud_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/HUD_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_HUD_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH HUD", w/2 - 110, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE RIGOR OF ISTIQAMAH, PROPHETIC RESILIENCE & THE DESTRUCTION OF TYRANTS", w/2 - 275, 305, font="F2", size=10.5, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of Uprightness, The Ark of Nuh, Defiance of Hud & Civilizational Reform Across Classical Sunni Exegetical Foundations", w/2 - 345, 288, font="F1", size=9, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("UPRIGHTNESS & PROPHETIC RESILIENCE", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("FASTAQIM KAMA UMIRTA & THE ARK", w/2 - box_w - 5, 227, font="F2", size=11, rgb=WHITE)
pdf.text("The Verse that Grayed the Prophet & Refusal to Lean Toward Tyrants", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Building the Ark Under Divine Watch (Bi-A'yunina) & Severed Bloodlines", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=ROSE, line_width=1.0)
pdf.text("DIVINE RETRIBUTION & CIVILIZATIONAL REFORM", w/2 + 25, 243, font="F2", size=9.5, rgb=ROSE)
pdf.text("THE RUINS OF TYRANNY & AHKAM", w/2 + 25, 227, font="F2", size=11, rgb=WHITE)
pdf.text("The Annihilation of 'Ad, Thamud, Sodom & Madyan After Rejection", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Economic Integrity (Baqiyyatullah) & Reformers Preserving Society", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH HUD MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("PERFECTED SCRIPTURE, PROVISION, CHALLENGE & THE ARK", c1_x + 12, 365, font="F2", size=9, rgb=CYAN)
pdf.text("Plates 01 to 04: Divine Sustenance, Human Despair, Ten Surahs & The Cosmic Deluge", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Perfected Book, Sustenance & Water", "Plate 03", "Uhkimat/Fussilat, Rizq guaranteed, Throne over Water, Conduct test"),
    ("Plate 04 : Psychological Dynamics & Ten Surahs", "Plate 04", "Despair vs Boasting, Sabr anchor, Ten fabricated chapters, Sensory parables"),
    ("Plate 05 : Nuh: Oligarchic Sneers & The Ark", "Plate 05", "Aradhiluna contempt, Refusing to expel poor, Desert shipyard, Sarcasm returned"),
    ("Plate 06 : Cosmic Deluge, Drowned Son & Judi", "Plate 06", "Boiling Tannoor, Mountain waves, Drowned son, Ya Ardu-bla'ee, Faith over blood")
]
...
pdf.text("PROPHETIC STRUGGLES, ECONOMIC ETHICS & ISTIQAMAH", c2_x + 12, 365, font="F2", size=9, rgb=GOLD)
pdf.text("Plates 05 to 08: 'Ad & Thamud, Sodom, Shu'ayb's Manifesto & The Rigor of Uprightness", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Hud to 'Ad & Salih to Thamud", "Plate 07", "Forelock formula, Miraculous She-Camel, Sonic Blast (As-Sayhah)"),
    ("Plate 08 : Ibrahim's Envoys & Sodom Overturned", "Plate 08", "Roasted calf, Sarah's wonder, Lut's agony, Seeking Ruknin Shadeed, Sijjeel"),
    ("Plate 09 : Economic Justice & Ruins of Tyranny", "Plate 09", "Market fraud forbidden, Baqiyyatullah, In ureedu illal-islah, Pharaoh to Fire"),
    ("Plate 10 : The Graying Mandate & The Return", "Plate 10", "Fastaqim kama umirta, Wa la tarkanoo, Prayers erase sins, Reformers save cities"),
...
pdf.text("SURAH HUD FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/hud_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/hud_compendium_page-01.png", os.path.join(brain_dir, "hud_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/hud_compendium_page-02.png", os.path.join(brain_dir, "hud_compendium_toc.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/hud_compendium_page-01.png", f"{PREVIEWS_DIR}/hud_compendium_cover.png")
shutil.copyfile(f"{PREVIEWS_DIR}/hud_compendium_page-02.png", f"{PREVIEWS_DIR}/hud_compendium_toc.png")
