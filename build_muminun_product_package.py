#!/usr/bin/env python3
"""
Huurs Studio - Surah Al-Mu'minun Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/muminun_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/MUMINUN_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_MUMINUN_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH AL-MU'MINUN", w/2 - 140, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE ANATOMY OF FAITH, MORPHOGENETIC DESIGN & THE BARZAXH BARRIER", w/2 - 290, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of The Hallmarks of Falah, Sequential Gestation, Prophetic Caravans, The Trembling Hearts, and The Finality of Barzakh Across 3 Foundation Lectures", w/2 - 380, 288, font="F1", size=8.5, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("THE SEVEN HALLMARKS & GESTATION", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("THE ANATOMY OF TRUE FALAH", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Khushu' in Prayer, Shunning Laghw & Fidelity to Trusts", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Clay to Ensoulment, Ahsanul-Khaliqeen & Firdaus Inheritance", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("TREMBLING HEARTS & THE BARZAXH", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("THE PANIC OF DEATH & THE SCALES", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("Giving While Hearts Tremble (Wajilah) & Pure Halal Diet", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Futile Plea Rabbi Irji'oon, The Chilling Kalla & The Barzakh", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AL-MU'MINUN MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("HALLMARKS OF FAITH, BIOLOGY & PROPHETS", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: The Decalogue, Gestation Stages, Cosmic Canopy & Prophetic Ark", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : The Opening Decalogue & Architecture of Falah", "Plate 03", "Qad Aflaha, Khushu', Laghw Shunning, Chastity & Firdaus"),
    ("Plate 04 : Embryological Morphogenesis & Resurrection", "Plate 04", "Sulalah min Teen, Mudghah, Bone Cladding & Rebirth"),
    ("Plate 05 : The Cosmic Canopy & Sustaining Provisions", "Plate 05", "Seven Paths, Measured Rain, Aquifers, Sinai Olive & Milk"),
    ("Plate 06 : The Prophetic Caravan Across Ages", "Plate 06", "Nuh's Ark, The Overthrown Generations, Musa & Maryam's Haven")
]
...
pdf.text("PURITY, THE BARZAXH & PURPOSEFUL UNIVERSE", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Trembling Hearts, Death's Panic, The Scales & Ultimate Mercy", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Commission of Purity & Sincere Hearts", "Plate 07", "Pure Food, Single Ummah, Qulubuhum Wajilah & Racing to Good"),
    ("Plate 08 : The Anatomy of Pride & The Barzakh", "Plate 08", "Turning on Heels, Rabbi Irji'oon, The Chilling Kalla & The Barrier"),
    ("Plate 09 : The Sounding Horn & The Cosmic Scales", "Plate 09", "Dissolved Kinship, Heavy Scales, Hellfire Agony & Great Rebuke"),
    ("Plate 10 : The Purposeful Universe & The Final Liturgy", "Plate 10", "The Ephemeral Day, Refuting 'Abathan & Rabbi Ighfir Warham"),
...
pdf.text("SURAH AL-MU'MINUN FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/muminun_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/muminun_compendium_page-01.png", os.path.join(brain_dir, "muminun_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/muminun_compendium_page-02.png", os.path.join(brain_dir, "muminun_compendium_toc.png"))
