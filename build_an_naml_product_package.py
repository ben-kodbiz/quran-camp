#!/usr/bin/env python3
"""
Huurs Studio - Surah An-Naml Product Package Compiler
Produces:
...
INTRO_PDF = "/tmp/an_naml_intro.pdf"
CONTENT_PDF = os.path.join(BASE_DIR, "07_MINDMAP/AN_NAML_MASTER_MINDMAP.pdf")
MASTER_COMPENDIUM_PDF = os.path.join(PRODUCTS_DIR, "SURAH_AN_NAML_MASTER_COMPENDIUM.pdf")
...
pdf.text("SURAH AN-NAML", w/2 - 115, 325, font="F2", size=26, rgb=WHITE)
pdf.text("THE DOMINION OF WISDOM: PROPHETIC POWER, GRATITUDE & COSMIC ORDER", w/2 - 290, 305, font="F2", size=9.8, rgb=GOLD_LIGHT)
pdf.text("The Definitive Visual Knowledge Architecture of The Burning Bush, The Ant's Excusing Compassion, Sheba's Throne, The Paved Crystal Palace, The 5 Cosmic Questions, and Moving Mountains Across 4 Movements", w/2 - 380, 288, font="F1", size=7.8, rgb=TEXT_MUTED)

# Showcase Badges
box_w = 320
pdf.rect(w/2 - box_w - 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=CYAN, line_width=1.0)
pdf.text("WISDOM, THE ANTS & SABA", w/2 - box_w - 5, 243, font="F2", size=9.5, rgb=CYAN)
pdf.text("SULAYMAN & THE HOOPOE SCOUT", w/2 - box_w - 5, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("The Language of Birds, Valley of Ants, Awzi'ni Du'a", w/2 - box_w - 5, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("Innahu Min Sulaymana, Throne Translocation & The Crystal Floor", w/2 - box_w - 5, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)

pdf.rect(w/2 + 15, 185, box_w, 75, fill_rgb=NAVY_ELEVATED, stroke_rgb=GOLD, line_width=1.0)
pdf.text("COSMOLOGY & SUN'ALLAH", w/2 + 25, 243, font="F2", size=9.5, rgb=GOLD)
pdf.text("A-ILAHUN MA'ALLAH? & MOUNTAINS", w/2 + 25, 227, font="F2", size=10.5, rgb=WHITE)
pdf.text("5 Cosmic Interrogations, Answering the Broken Soul (Al-Mudtarr)", w/2 + 25, 212, font="F1", size=8, rgb=TEXT_MUTED)
pdf.text("The Moving Mountains (Marra as-Sahab) & Atqana Kulla Shay'", w/2 + 25, 198, font="F3", size=7.5, rgb=GOLD_LIGHT)
...
pdf.text(" |  SURAH AN-NAML MASTER COMPENDIUM", 122, h - 26, font="F1", size=8.5, rgb=TEXT_MUTED)
...
pdf.text("SACRED LIGHT, WISDOM & THE ANTS", c1_x + 12, 365, font="F2", size=8.8, rgb=CYAN)
pdf.text("Plates 01 to 04: The Fire of Tuwa, Language of Birds, The Hoopoe & Crystal Floor", c1_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
...
col1_items = [
    ...
    ("Plate 03 : Sacred Illumination & Musa at the Fire", "Plate 03", "Ta-Seen, Burning Bush, Agile Serpent & Pharaoh's Arrogance"),
    ("Plate 04 : Dawud, Sulayman & Language of Creation", "Plate 04", "Gift of Knowledge, Valley of Ants & The Awzi'ni Du'a"),
    ("Plate 05 : The Hoopoe & The Kingdom of Sheba", "Plate 05", "Airborne Reconnaissance, Solar Idolatry & Innahu Min Sulayman"),
    ("Plate 06 : Translocation & The Crystal Pavilion", "Plate 06", "Book Knowledge, Twinkling of Eye, Hadha Min Fadli & Bilqis")
]
...
pdf.text("CONSPIRACIES, COSMOLOGY & PRAISE", c2_x + 12, 365, font="F2", size=8.8, rgb=GOLD)
pdf.text("Plates 05 to 08: Nine Plotters, Cosmic Questions, Moving Mountains & Alhamdulillah", c2_x + 12, 352, font="F3", size=7.2, rgb=TEXT_MUTED)
pdf.line(c2_x, 345, c2_x + c_w, 345, stroke_rgb=GOLD, line_width=0.8)

col2_items = [
    ("Plate 07 : Salih's Thamud & Nine Conspirators", "Plate 07", "Tis'atu Raht, Night Assassination Plot & Desolate Ruin"),
    ("Plate 08 : Five Cosmic Interrogations", "Plate 08", "A-Ilahun Ma'allah?, Answering the Mudtarr & Earth Succession"),
    ("Plate 09 : Eschatological Signs & Moving Mountains", "Plate 09", "The Beast (Dabbah), Horn Blast, Mountains as Clouds & Sun'allah"),
    ("Plate 10 : Prophetic Mission, Dhikr & Supreme Praise", "Plate 10", "Lord of Mecca, Reciting the Qur'an & Universal Praise"),
...
pdf.text("SURAH AN-NAML FOUNDATION ARCHITECTURE", w - 240, 13, font="F2", size=7.2, rgb=GOLD)
...
# Generate Previews
cmd_prev = f"pdftoppm -png -r 150 -f 1 -l 2 {MASTER_COMPENDIUM_PDF} {PREVIEWS_DIR}/an_naml_compendium_page"
subprocess.run(cmd_prev, shell=True, check=True)

# Copy to brain dir
shutil.copyfile(f"{PREVIEWS_DIR}/an_naml_compendium_page-01.png", os.path.join(brain_dir, "an_naml_compendium_cover.png"))
shutil.copyfile(f"{PREVIEWS_DIR}/an_naml_compendium_page-02.png", os.path.join(brain_dir, "an_naml_compendium_toc.png"))
